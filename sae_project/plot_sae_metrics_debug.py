...
def independent_dimension_diagnostic():
    ...
    if len(logs_dims) <= 1:
        print(
            "    ❌ [경고] 구글 드라이브 원본 파일 자체에 차원이 1종류뿐이거나 찾지 못했습니다."
        )
    ...
    # -------------------------------------------------------------------------
    linear_csv_path = (
        os.path.join(SAVE_DIR, LINEAR_PROBE_CSV)
        if not os.path.isabs(LINEAR_PROBE_CSV)
        else LINEAR_PROBE_CSV
    )
    acc_dims = set()
    acc_count = 0
            ...
            if "File" in df_acc.columns and (len(acc_dims) <= 1 or 600 in acc_dims):
                ...
                for fpath in df_acc["File"].astype(str):
                    if "cnn_gap" in fpath or "proxy" in fpath.lower():
                        ...
                        if m:
                            simulated_dims.add(int(m.group(1)))
                if simulated_dims:
    ...
    if len(acc_dims) <= 1:
        print(
            "    ❌ [경고] 정확도 CSV에서 인식된 차원이 1종류(혹은 기본값 600)뿐입니다."
        )
    ...
    if logs_count == 0:
        print(
            "❌ 원인: 구글 드라이브 내 'all_experiments_summary.csv'나 'trainlog.csv'를 하나도 찾지 못했습니다."
        )
        print(
            "   👉 해결책: `BASE_DIR` 경로가 올바른지, 코랩에 구글 드라이브 마운트가 정상 처리되었는지 확인하세요."
        )
        return

    if len(logs_dims) <= 1 and len(acc_dims) <= 1:
        print(
            "❌ 원인: 수집된 모든 파일에 차원(Dimension)이 실제로 1종류밖에 없습니다."
        )
        print(
            "   👉 해결책: 실험 데이터가 의도대로 여러 차원(예: 512, 1024 등)으로 추출되었는지 폴더 구조를 직접 확인해야 합니다."
        )
        return

    if len(logs_dims) > 1 and len(acc_dims) <= 1:
        print(
            "❌ 원인: 원본 데이터에는 여러 차원이 있으나, 'Linear Probe 정확도 CSV'를 병합하는 과정에서 차원이 유실되었습니다."
        )
        print(
            "   👉 원인 세부: 기존 코드의 `if 'SAE_dim' not in fpath:` 조건 때문에 파일명을 제대로 읽지 못해 모든 차원이 '600'으로 강제 초기화되었을 확률이 매우 높습니다."
        )
        print(
            "   👉 해결책: `sae_linear_probe_1e5_results.csv` 파일 내부의 'File' 컬럼 경로에 'SAE_dim' 문자열이 정확히 포함되어 있는지 확인하세요."
        )
        return
    ...
    if str in mix_check_logs or str in mix_check_acc:
        ...
        print(
            "   👉 특정 파일에서는 차원이 문자열('600')로, 다른 곳에서는 숫자(600)로 저장되어 판다스가 병합(`merge`)에 실패했습니다."
        )
        print(
            "   👉 해결책: 데이터 병합 직전 양쪽 데이터프레임의 Dimension을 `.astype(int)`로 통일해야 합니다."
        )
        return
    ...
    print(
        "   👉 루프 내부에서 `color=palette[i]`를 맵핑할 때, 에러바 플롯 구조상 색상이 단일 색상으로 덮어써지고 있습니다."
    )
    print(
        "   👉 해결책: 그래프 함수 내부의 컬러 설정을 `color=plt.cm.Set1(i)` 나 `plt.cm.viridis(i / len(dimensions))` 형태로 명시적 변경해야 합니다."
    )
