...
def plot_heatmaps(data_dict, title_prefix, save_path_base, gamma=1.0):
    ...
    for idx, (label, sim_mat) in enumerate(data_dict.items()):
        ...
        norm = SymmetricPowerNorm(gamma=gamma, vmin=-1.0, vmax=1.0)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Superposition Disentanglement from SAE decoder weights.")
    parser.add_argument("--base_dir", type=str, default="/content/drive/MyDrive/Final_paper/lambda_labs_moco_only")
    parser.add_argument("--heatmap_gamma", type=float, default=2.5,
                   help="Gamma exponent for heatmap color scaling (power law). Default 2.5 (enhanced contrast)")
    parser.add_argument("--save_dir", type=str, default="/content/drive/MyDrive/Final_paper/lambda_labs_moco_only/caches_per_image_centering/disentanglement")
    ...
    if not df_dim.empty:
        ...
        sns.pointplot(data=df_dim, x="Config", y="Max_IP", color="red", label="Max Inner Product", capsize=.1, errorbar="sd")
        
        plt.title("Superposition Disentanglement vs Dimension (Fixed λ=800)", fontsize=14, fontweight='bold')
    ...
    if not df_lam.empty:
        ...
        sns.pointplot(data=df_lam, x="Lambda_str", y="Max_IP", color="red", label="Max Inner Product", capsize=.1, errorbar="sd")
        
        plt.title("Superposition Disentanglement vs Sparsity (Fixed d=4096)", fontsize=14, fontweight='bold')
    ...
    if heatmaps_dim:
        ...
        plot_heatmaps(plot_dict, "Concept Vectors Inner Product Heatmaps vs Dimension (Seed 42)", 
                      os.path.join(args.save_dir, "Heatmaps_vs_Dimension"), gamma=args.heatmap_gamma)
    ...
    if heatmaps_lam:
        ...
        plot_heatmaps(plot_dict, "Concept Vectors Inner Product Heatmaps vs Sparsity (Seed 42)", 
                      os.path.join(args.save_dir, "Heatmaps_vs_Lambda"), gamma=args.heatmap_gamma)
