...
class SAETrainer:
    def __init__(
        ...
        self.sae = PointwiseTopKSAE(
            d_in=args.d_in, d_sae=args.d_sae, k=args.k, init_scale=args.sae_init_scale
        ).to(self.device)
        ...
        self.ckpt_path = os.path.join(
            args.sae_save_dir, f"sae_{args.which_layer}_d{args.d_sae}_k{args.k}.pt"
        )
        self.best_ckpt_path = os.path.join(
            args.sae_save_dir, f"sae_{args.which_layer}_BEST_d{args.d_sae}_k{args.k}.pt"
        )
        self.log_csv_path = os.path.join(
            args.sae_save_dir, f"sae_{args.which_layer}_trainlog.csv"
        )
        ...
        logger.info(
            f"[AuxK] aux_k={self.sae.aux_k}, aux_coeff={self.sae.aux_coeff} (training-only)"
        )
    ...
    def train(self):
        ...
        logger.info(
            f"[SAE] which_layer={self.args.which_layer}, d_in={self.args.d_in}, d_sae={self.args.d_sae}, k={self.args.k}"
        )
        logger.info(
            f"[SAE] tokens_per_image={self.args.tokens_per_image}, token_batch={self.token_batch}, shuffle_tokens={self.shuffle_tokens}"
        )
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            train_mse_sum, train_l1_sum, train_aux_mse_sum, train_steps = (
                0.0,
                0.0,
                0.0,
                0,
            ...
            pbar = tqdm(
                self.train_loader,
                desc=f"SAE Train E{epoch}/{self.args.epochs}",
                leave=True,
            )
            for batch in pbar:
                if batch is None:
                ...
                for s in range(0, n, Tb):
                    ...
                    loss = (
                        mse_main
                        + float(self.args.l1_coeff) * l1_main
                        + self.sae.aux_coeff * aux_mse
                    )
                    ...
                    with torch.no_grad():
                        self.sae.update_usage_ema_(
                            acts_main.detach(), ema=float(self.args.usage_ema)
                        )

                    mse_acc += float(mse_main.item())
                    l1_acc += float(l1_main.item())
                    aux_mse_acc += (
                        float(aux_mse.item()) if self.sae.aux_coeff > 0 else 0.0
                    )
                    chunks += 1
                ...
                # dead count
                dead_count = int(
                    (self.sae.usage_ema < float(self.args.dead_threshold)).sum().item()
                )
            ...
            # epoch summaries
            if train_steps == 0:
                train_mse_avg = train_l1_avg = train_aux_mse_avg = train_total_main = (
                    train_fvu
                ) = 0.0
            else:
                ...
                train_total_main = (
                    train_mse_avg + float(self.args.l1_coeff) * train_l1_avg
                )
                train_fvu = 0.0
                if getattr(self.args, "log_fvu", False):
                    train_fvu = (
            ...
            if self.val_loader is not None:
                val_mse, val_l1, val_aux_mse, val_total_main, val_fvu = self.eval_epoch(
                    self.val_loader, "Val"
                )
            ...
            metric = (
                val_total_main if (self.val_loader is not None) else train_total_main
            )
            ...
            if getattr(self.args, "save_best", False):
                if metric < self.best_metric:
                    ...
                    tqdm.write(
                        f"  -> Saved BEST to {self.best_ckpt_path} (metric(main_total)={self.best_metric:.6f})"
                    )
