...
class GatedSAE(nn.Module):
    ...
    def _compute_aux_activations(
        ...
        _, idx = torch.topk(
            masked_mag, k=min(aux_k, self.d_sae), dim=1, largest=True, sorted=False
        )
    ...
    def load_clustering_init(
        ...
        with torch.no_grad():
            ...
            if n_centroids >= self.d_sae:
                ...
                W_init[:, n_centroids:] = (
                    np.random.randn(self.d_in, self.d_sae - n_centroids).astype(
                        np.float32
                    )
                    * 0.02
                )
            ...
            if noise_scale > 0:
                W_init += (
                    np.random.randn(*W_init.shape).astype(np.float32) * noise_scale
                )
