...
def get_aux_coeff_cosine_decay(
    epoch: int,
    total_epochs: int,
    initial_aux: float,
    decay_start_epoch: int = 2,
) -> float:
    """
    Aux coefficient cosine decay: epoch 2부터 마지막 epoch까지 0으로 감소.
    
    - Epoch 1 ~ decay_start_epoch-1: initial_aux 유지
    - decay_start_epoch ~ total_epochs: cosine decay → 0
    
    예시 (epochs=4, initial=0.03125):
      E1: 0.03125, E2: 0.0234, E3: 0.0078, E4: 0.0
    """
    import math
    ...
    # Epoch 2 ~ total_epochs 구간에서 0~1로 진행
    decay_epochs = total_epochs - decay_start_epoch + 1
    progress = (epoch - decay_start_epoch) / (decay_epochs - 1) if decay_epochs > 1 else 1.0
    progress = min(1.0, progress)
    
    # Cosine: 1 → 0
    cosine_factor = 0.5 * (1 + math.cos(math.pi * progress))
    
    return initial_aux * cosine_factor
