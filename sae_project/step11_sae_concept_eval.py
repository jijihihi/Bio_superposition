...
def compute_concept_purity_and_entropy(
    ...
    stats = {
        ...
        "purity_median": (
            float(np.median(valid_purity)) if len(valid_purity) > 0 else 0.0
        ),
        "purity_p90": (
            float(np.percentile(valid_purity, 90)) if len(valid_purity) > 0 else 0.0
        ),
        "purity_p95": (
            float(np.percentile(valid_purity, 95)) if len(valid_purity) > 0 else 0.0
        ),
        "entropy_mean": (
            float(np.mean(valid_entropy)) if len(valid_entropy) > 0 else 0.0
        ),
...
def compute_token_activation_pmf(
    ...
    if len(active_counts) == 0:
        return (
            np.array([0]),
            np.array([1.0]),
            {"gini": 0.0, "mean": 0.0, "normalized_mean": 0.0},
        )
    ...
    stats = {
        ...
        "p1": (
            float(pmf[0]) if len(pmf) > 0 else 0.0
        ),  # Fraction activating exactly 1 concept
        "p2": (
            float(pmf[1]) if len(pmf) > 1 else 0.0
        ),  # Fraction activating exactly 2 concepts
        "p3+": (
            float(pmf[2:].sum()) if len(pmf) > 2 else 0.0
        ),  # Fraction activating 3+ concepts
    }
...
def compute_gini(values: np.ndarray) -> float:
    ...
    return (2 * np.sum((np.arange(1, n + 1) * values)) - (n + 1) * cumulative[-1]) / (
        n * cumulative[-1]
    )
...
def evaluate_sae_concepts(
    ...
    # Storage
    concept_class_counts = defaultdict(
                    ...
                    if num_active > 0:
                        ...
                        for concept_id in active_concepts:
                            ...
                            if current_image_idx not in concept_image_sets[concept_id]:
                                ...
                                concept_image_class[(concept_id, current_image_idx)] = (
                                    class_idx
                                )
...
def print_evaluation_results(results: Dict, layer: str):
    ...
    print(
        f"  Active tokens:    {results['active_tokens']:,} ({results['active_token_ratio']*100:.2f}%)"
    )
    ...
    print(
        f"  Concepts/token:   {ts['mean_concepts_per_token']:.2f} ± {ts['std_concepts_per_token']:.2f}"
    )
...
def plot_evaluation_results(results: Dict, save_path: str, layer: str):
    ...
    ax1.axvline(
        results["purity_stats"]["purity_mean"],
        color="green",
        linestyle="-",
        label=f'Mean={results["purity_stats"]["purity_mean"]:.3f}',
    )
    ...
    ax2.axvline(
        results["purity_stats"]["entropy_mean"],
        color="green",
        linestyle="-",
        label=f'Mean={results["purity_stats"]["entropy_mean"]:.3f}',
    )
    ...
    if valid_mask.sum() > 0:
        ax4.scatter(
            results["purity"][valid_mask],
            results["entropy"][valid_mask],
            alpha=0.3,
            s=5,
        )
...
def save_results_csv(results: Dict, save_path: str):
    ...
    with open(save_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            ["concept_id", "purity", "entropy", "total_images"]
...
def main(args_list=None):
    ...
    ds = InMemorySixteenBitDataset(
        bank, list(range(len(refidx))), args.img_size, augment=False
    )
    loader = DataLoader(
        ds,
