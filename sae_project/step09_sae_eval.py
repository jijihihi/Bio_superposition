...
def evaluate_concepts_for_sae(
    ...
    # Save per-image GAP cache (StrictPlateBalanced batch centering)
    if per_image_gap_list:
        X_all_gap = np.concatenate(per_image_gap_list, axis=0).astype(np.float32)
        y_arr = np.array(per_image_y_list, dtype=np.int64)
        lines_arr = np.array(per_image_line_list, dtype=object)
        uids_arr = np.array(per_image_uid_list, dtype=object)
        usage_ema_np = sae.usage_ema.cpu().numpy()

        gap_npz_path = gap_csv_path.replace(".csv", "_per_image.npz")
        np.savez_compressed(
            gap_npz_path,
            X_all=X_all_gap,
            y=y_arr,
            lines=lines_arr,
            uids=uids_arr,
            usage_ema=usage_ema_np,
            which_layer=np.array(args.which_layer),
        )
        logger.info(f"  Saved per-image GAP cache: {gap_npz_path}")
        logger.info(f"    X_all: {X_all_gap.shape} ({X_all_gap.nbytes / 1e6:.1f} MB)")
        del X_all_gap, y_arr, lines_arr, uids_arr
