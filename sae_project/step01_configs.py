...
def get_args(args_list=None):
    ...
    p.add_argument("--token_norm_mode", type=str, default="gap-scalar",
                   choices=["per-token", "gap-scalar"], # 지금 기본적으로 그 레이어 GAP L2 norm으로 각 그리드 모두 나눠 준 후에 각 토큰도 L2 norm으로 나눠주는데 loss function mse에서 그 토큰 L2 norm 곱해줘서 L2 norm 큰거에 집중하도록 유도.
