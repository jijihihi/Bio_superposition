...
def get_args():
    ...
    p.add_argument("--phate_decay", type=int, default=40, help="PHATE alpha decay parameter")
    p.add_argument("--phate_t", type=str, default="auto", help="PHATE t parameter (int or 'auto')")
    p.add_argument("--leiden_resolution", type=float, default=1.0, help="Leiden clustering resolution")
    ...
    p.add_argument("--paga_figsize", type=float, nargs=2, default=[6.0, 6.0], help="Figure size for PAGA plot (width height)")
    
    # Plotting arguments
    p.add_argument("--plot_size", type=float, default=1.5, help="Point size for global PHATE scatter")
    p.add_argument("--plot_alpha", type=float, default=0.3, help="Point alpha for global PHATE scatter")
...
def plot_phate(phate_coords, labels, output_path, dpi=200, point_size=1.5, alpha=0.3):
    ...
    for cls in plot_order:
        ...
        ax.scatter(phate_coords[mask, 0], phate_coords[mask, 1], s=point_size, alpha=alpha,
                   label=f"{cls} (n={mask.sum():,})", c=color, edgecolors="none", rasterized=True)
...
def run_global_vis(args):
    ...
    out_dir = args.output_dir or os.path.join(os.path.dirname(args.features_cache), "global_vis")
    ...
    if phate_lib is not None:
        ...
        plot_phate(X_phate, superclasses, phate_out, dpi=args.dpi, point_size=args.plot_size, alpha=args.plot_alpha)
    else:
    ...
    sc.tl.leiden(adata, resolution=args.leiden_resolution, random_state=args.seed, flavor='igraph', directed=False)
    ...
    save_crosstab_as_svg(composition, ct_path, dpi=args.dpi, title="Global PAGA Cluster Composition")
    ...
    if edges:
        ...
        logger.info(f"    Global PAGA Connectivities (>={args.paga_threshold}):\n      " + "\n      ".join(edge_strs))
    else:
        logger.info(f"    Global PAGA Connectivities: No edges above threshold {args.paga_threshold}")
    
    fig, ax = plt.subplots(figsize=tuple(args.paga_figsize))
    sc.pl.paga(
        adata, ax=ax, show=False, title="Global PAGA (Leiden)",
...
if __name__ == "__main__":
    args = get_args()
    if not args.norm: args.norm = "log_std"  # Default for explicit run
    run_global_vis(args)
