...
def get_args():
    p = argparse.ArgumentParser(description="DPT-ordered feature module activations heatmap")
    ...
    p.add_argument("--plot_features", type=str, nargs="*", default=[], help="e.g. 'SNCA:1123' or '1123'")
...
def plot_dpt_feature_heatmap(dpt_vals, X_features, class_labels, mutation, out_dir, prefix, dpi=200, n_bins=100, seed=42):
    valid = np.isfinite(dpt_vals)
    if valid.sum() < n_bins:
        logger.warning(f"    Not enough valid cells ({valid.sum()}) for {n_bins} bins. Skipping heatmap.")
        return
    ...
    sc.tl.leiden(adata_feat, resolution=1.0, random_state=seed, flavor='igraph', directed=False)
    ...
    im = ax.imshow(X_heatmap_final, aspect='auto', cmap='RdBu_r', vmin=-2, vmax=2, interpolation='nearest')
    
    ax.set_xlabel("Diffusion Pseudotime (Binned)", fontsize=12)
    ax.set_ylabel(f"All {X_features.shape[1]} DE Feature Maps (Leiden Clustered)", fontsize=12)
    ax.set_title(f"Feature Module Activations along DPT (Control vs {mutation})", fontsize=14)
    ax.set_yticks([])
    ...
    fig.savefig(hm_path.replace(".png", ".svg"), format="svg", bbox_inches="tight", transparent=True)
    if _IN_COLAB: plt.show()
    plt.close(fig)
    logger.info(f"    Heatmap saved: {hm_path}")

def plot_feature_trend(dpt_vals, feat_vals, class_labels, mutation, out_dir, prefix, dpi=200, gam_trim_pctl=[1, 99], alpha=0.3, point_size=10.0):
    valid = np.isfinite(dpt_vals)
    ...
    n_points = len(dpt_v)
    low_idx, high_idx = int(n_points * (gam_trim_pctl[0] / 100.0)), int(n_points * (gam_trim_pctl[1] / 100.0))
    if high_idx <= low_idx + 10: low_idx, high_idx = 0, n_points
    ...
    ax.scatter(dpt_fit, feat_fit, color=scatter_color, alpha=alpha, s=point_size, label=mutation, edgecolors="none")
    
    try:
        from pygam import LinearGAM, s as s_term
        gam = LinearGAM(s_term(0, n_splines=10, spline_order=3)).fit(dpt_fit.reshape(-1, 1), feat_fit)
        ...
        ax.fill_between(x_line, ci[:, 0], ci[:, 1], color="black", alpha=0.15, zorder=4, linewidth=0)
    except ImportError:
        sort_idx = np.argsort(dpt_v)
...
def run_dpt_heatmap(args):
    ...
    if args.plot_features:
        ...
        from trajectory_utils import load_features_cache, apply_normalization
        if "X_gap" in data: X_raw_all = data["X_gap"]
        else: X_raw_all, _, _, _, _, _ = load_features_cache(args.features_cache, args.dead_threshold)
        ...
        X_raw_norm = apply_normalization(X_raw_all, args.norm) if args.norm and args.norm != "none" else X_raw_all
        
        alive_indices = np.where(data.get("usage_ema", np.ones(X_raw_all.shape[1])) >= args.dead_threshold)[0]
        for f_str in args.plot_features:
            target_mut, fid_str = f_str.split(":") if ":" in f_str else ("ALL", f_str)
            fid = int(fid_str)
            if fid in alive_indices:
                plot_feature_idxs[fid] = (np.where(alive_indices == fid)[0][0], target_mut)

    out_dir = args.output_dir or os.path.join(os.path.dirname(args.features_cache), "dpt_heatmap")
    ...
    for mut in ["SNCA", "GBA", "LRRK2"]:
        ...
        plot_dpt_feature_heatmap(dpt_mut, X_mut_features, sc_mut_only, mutation=mut, out_dir=out_dir, 
                                 prefix=f"{args.norm}_{which_layer}", dpi=args.dpi, n_bins=args.n_bins, seed=args.seed)
        ...
        if X_raw_norm is not None:
            ...
            for fid, (col_idx, target_mut) in plot_feature_idxs.items():
                if target_mut not in ("ALL", mut): continue
