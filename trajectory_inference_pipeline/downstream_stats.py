...
def get_args():
    p = argparse.ArgumentParser(
        description="Downstream DPT Stats: terciles, spread, robustness"
    )
...
def diagnose_dpt_distribution(dpt_all, superclasses_arr, out_dir, prefix, dpi=200):
    ...
    for grp in ["Control"] + mutations:
        ...
        ax.hist(dpt_v, bins=60, alpha=0.85, color=c, edgecolor="black", linewidth=0.3)
        ax.set_title(f"{grp} DPT Distribution (spread={spread:.4f})")
        fig.tight_layout()
        sns.despine()
        fig.savefig(
            os.path.join(out_dir, f"dpt_dist_{grp}_{prefix}.svg"),
            dpi=dpi,
            bbox_inches="tight",
        )
        if _IN_COLAB:
...
def plot_jt_terciles(dpt_vals, apop_vals, mutation, output_path, dpi=200, n_terciles=3):
    ...
    tercile_labels = (
        ...
        if n_terciles_jt == 3
        else [f"Q{i+1}" for i in range(n_terciles_jt)]
    )

    group_data = []
    for i in range(n_terciles_jt):
        if i == n_terciles_jt - 1:
            mask_t = (dpt_vals >= tercile_edges[i]) & (dpt_vals <= tercile_edges[i + 1])
        else:
            mask_t = (dpt_vals >= tercile_edges[i]) & (dpt_vals < tercile_edges[i + 1])
    ...
    try:
        ...
        for i in range(len(group_data)):
            for j in range(i + 1, len(group_data)):
                ...
                n_less = (
                    idx_left  # count how many elements in gj are strictly LESS than gi
                )
        ...
        var_jt = (
            N**2 * (2 * N + 3) - sum(n**2 * (2 * n + 3) for n in ns)
        ) / 72 - tie_term / 72
        z_jt = (
            (jt_stat - mu_jt - 0.5) / np.sqrt(max(var_jt, 1e-12))
            if jt_stat > mu_jt
            else (jt_stat - mu_jt + 0.5) / np.sqrt(max(var_jt, 1e-12))
        )
    ...
    comparisons = (
        [
            (0, 1, "Proximal", "Intermediate"),
            (1, 2, "Intermediate", "Distal"),
            (0, 2, "Proximal", "Distal"),
        ]
        if n_terciles_jt == 3
        else [(i, i + 1, f"Q{i+1}", f"Q{i+2}") for i in range(n_terciles_jt - 1)]
    )
    ...
    for i_grp, j_grp, li, lj in comparisons:
        ...
        if len(gi) < 2 or len(gj) < 2:
            continue
        try:
            _, p_raw = mannwhitneyu(gi, gj, alternative="two-sided")
        except ValueError:
            p_raw = 1.0
    ...
    # Plot
    fig_jt, ax_jt = plt.subplots(figsize=(3.5, 4))
    ...
    ax_jt.bar(
        ...
        capsize=5,
        width=0.6,
        color=colors_tercile[:n_terciles_jt],
        alpha=0.85,
        edgecolor="black",
        linewidth=0.8,
    )
    ...
    for ci, (i_grp, j_grp, li, lj) in enumerate(comparisons):
        ...
        h = (y_max * 0.08) * 0.3
        ax_jt.plot(
            [x1, x1, x2, x2], [y_b - h, y_b, y_b, y_b - h], lw=1.2, color="black"
        )
        ax_jt.text(
            (x1 + x2) / 2,
            y_b + h * 0.3,
    ...
    fig_jt.savefig(
        output_path.replace(".png", ".svg"), format="svg", bbox_inches="tight"
    )
    if _IN_COLAB:
        plt.show()
    plt.close(fig_jt)
...
def run_downstream_stats(args):
    ...
    for mut in ["SNCA", "GBA", "LRRK2"]:
        ...
        # Diagnostics
        diagnose_dpt_distribution(
            dpt_pair,
            pair_sc,
            out_dir,
            prefix=f"{args.norm}_{which_layer}_{mut}",
            dpi=args.dpi,
        )
        ...
        if valid.sum() > 10:
            jt_path = os.path.join(
                out_dir, f"terciles_jt_{args.norm}_{which_layer}_{mut}.png"
            )
            plot_jt_terciles(
                dpt_mut[valid],
                apop_mut[valid],
