...
def get_args():
    p = argparse.ArgumentParser(description="Aggregate generated trajectory figures into a single panel.")
    p = add_trajectory_arguments(p)
    p.add_argument("--vis_dir", type=str, default="", help="Directory containing PHATE/PAGA pngs")
    p.add_argument("--dpt_dir", type=str, default="", help="Directory containing DPT Scatter pngs")
    p.add_argument("--stats_dir", type=str, default="", help="Directory containing Terciles Bar pngs")
    return p.parse_args()

def run_aggregate_figures(args):
    mutations = ["SNCA", "GBA", "LRRK2"]
    plot_types = ["PHATE", "DPT Scatter", "Apoptosis by Stage", "PAGA"]
    ...
    vis_dir = args.vis_dir if args.vis_dir else (os.path.join(base_dir, "pairwise_vis") if not args.output_dir else base_dir)
    dpt_dir = args.dpt_dir if args.dpt_dir else (os.path.join(base_dir, "pairwise_dpt") if not args.output_dir else base_dir)
    stats_dir = args.stats_dir if args.stats_dir else (os.path.join(base_dir, "downstream_stats") if not args.output_dir else base_dir)
    ...
    def get_img_path(plot_type, mut):
        prefix = f"{args.norm}_{args.which_layer}_{mut}" if hasattr(args, 'which_layer') else f"{args.norm}_none_{mut}"
    ...
    for i, mut in enumerate(mutations):
        ...
        for j, (plot_type, img_path) in enumerate(zip(plot_types, paths)):
            ...
            # Row titles
            if j == 0:
                ax.text(-0.1, 0.5, mut, fontsize=20, fontweight='bold', va='center', ha='right', transform=ax.transAxes, rotation=90)
            ...
            if os.path.exists(img_path):
                ...
                ax.text(0.5, 0.5, f"Missing:\n{os.path.basename(img_path)}", ha='center', va='center', fontsize=12, color='red')
