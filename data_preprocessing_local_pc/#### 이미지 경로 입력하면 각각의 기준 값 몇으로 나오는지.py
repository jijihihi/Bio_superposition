...
def visualize_background_mask(file_path):
    ...
    for i in range(c):
        ...
        if raw_std < CONFIG['min_std_threshold']:
            ...
            if cutoff <= 0: scale_factor = 0
            else: scale_factor = target_max / cutoff
            
            print(f"  - Ch{i}: Std={raw_std:.1f}, Cutoff={cutoff:.0f} -> Factor={scale_factor:.2f}x")
    ...
    print("👉 이 이미지를 열어보세요. 흰색이 아닌 부분이 전부 '세포'로 인식된 영역입니다.")
