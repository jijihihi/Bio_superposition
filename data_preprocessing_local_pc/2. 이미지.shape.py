...
def check_image_data_integrity(root_dir):
    ...
    if dataset_summary:
        ...
        print(f"  - Shape Consistency: {'✅ Passed' if shape_consistent else f'⚠️ Failed (Found {len(set(shapes))} variations)'}")
        print(f"  - Dtype Consistency: {'✅ Passed' if dtype_consistent else f'⚠️ Failed (Found {len(set(dtypes))} variations)'}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check integrity and consistency of TIFF image datasets.")
    parser.add_argument("--dir", type=str, default=r"C:\Users\admin\Desktop\MIP", help="Path to the dataset root directory")
