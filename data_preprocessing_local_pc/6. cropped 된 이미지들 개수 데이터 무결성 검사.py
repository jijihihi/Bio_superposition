...
def run_verification(args):
    ...
    print(
        f"{'Folder Group':<40} | {'Source':<8} | {'Target':<8} | {'Exp. (x64)':<10} | {'Status'}"
    )
    print("-" * 90)
    ...
    for group, s_count in sorted(source_counts.items()):
        ...
        print(
            f"{group[:40]:<40} | {s_count:<8} | {t_count:<8} | {expected:<10} | {status}"
        )
        ...
        with tqdm(total=len(target_files_list), unit="img") as pbar:
            ...
            for fpath in target_files_list:
                try:
                    ...
                    if img.dtype != np.uint16:
                        failures.append(
                            f"{os.path.basename(fpath)}: Type {img.dtype} != uint16"
                        )
                        is_valid = False
                    ...
                    if is_valid:
                        if img.ndim != 3:
                            failures.append(
                                f"{os.path.basename(fpath)}: Not 3D {img.shape}"
                            )
                            is_valid = False
                        elif 3 not in img.shape:
                            failures.append(
                                f"{os.path.basename(fpath)}: No RGB channel found {img.shape}"
                            )
                            is_valid = False
