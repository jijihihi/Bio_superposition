...
def get_args():
    ...
    # 경로 설정
    parser.add_argument("--root_dir", type=str, default=r"D:\From_C_drive\MIP", help="Root directory containing image folders.")
    parser.add_argument("--csv_path", type=str, default=r"D:\From_C_drive\MIP\QC_Output\qc_metrics_raw_per_channel.csv", help="Path to the QC metrics CSV file.")
    ...
    parser.add_argument("--min_max_laplacian", type=float, default=800000.0, help="Reject if MAX Laplacian variance across channels is below this (Default: 8*10^5). you can change this value.")
    parser.add_argument("--min_avg_intensity", type=float, default=1000.0, help="Reject if AVERAGE intensity across channels is below this.")
    
    # 배경 필터링 설정
    parser.add_argument("--bg_pixel_threshold", type=float, default=1000.0, help="Sum of scaled RGB channels below this value is considered background.")
    parser.add_argument("--max_bg_fraction", type=float, default=0.70, help="Reject if background pixel fraction is above this (0.70 = 70%).")
...
def check_background_fraction(img_path, bg_pixel_thresh=1000, max_bg_frac=0.70):
    ...
    try:
        ...
        return False, 0.0 # Error safe: do not reject if read fails (manual check needed)
...
def run_filtering(args):
    ...
    print(f"❌ Rejected (Metrics):   {stats['Rejected_Metric']} (Low Sharpness/Intensity)")
    print(f"❌ Rejected (Backgrnd): {stats['Rejected_Background']} (Empty/Noise > {args.max_bg_fraction*100}%)")
