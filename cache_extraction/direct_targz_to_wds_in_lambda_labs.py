...
def convert_to_wds(source_dir, dest_root, limit):
    ...
    for tar_file in tar_files:
        ...
        def process_image(filename, img_data):
            ...
            # 5000장 단위로 샤드 교체
            if state['out_tar'] is None or state['count'] >= (state['shard_idx'] * images_per_shard):
                if state['out_tar'] is not None:
                    state['out_tar'].close()
                shard_path = os.path.join(wds_class_dir, f"{state['shard_idx']:04d}.tar")
            ...
            for outer_member in outer_tar:
                ...
                if outer_member.isfile() and outer_member.name.lower().endswith('.tar'):
                    ...
                    if outer_stream is not None:
                        ...
                        with tarfile.open(fileobj=io.BytesIO(tar_data), mode="r") as inner_tar:
                            for inner_member in inner_tar:
                                if class_count >= limit:
                                    break
                                if inner_member.isfile() and inner_member.name.lower().endswith(('.tif', '.tiff')):
                                    with inner_tar.extractfile(inner_member) as src:
                                        process_image(inner_member.name, src.read())

                # Case 2: .tar.gz 안에 바로 .tif가 있는 구조
                elif outer_member.isfile() and outer_member.name.lower().endswith(('.tif', '.tiff')):
        ...
        print(f"  ✅ [{class_name}] 총 {class_count}장 변환 완벽 완료. (총 {len(plates_state)}개 Plate 탐지됨)\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert tar.gz directly to WebDataset shards with JSON padding")
    parser.add_argument("--source_dir", type=str, default="/home/ubuntu/model-east3/wds_shards_tar", help="Directory with .tar.gz")
    parser.add_argument("--dest_dir", type=str, default="/home/ubuntu/model-east3/wds_shards_tar", help="WDS Target Root")
