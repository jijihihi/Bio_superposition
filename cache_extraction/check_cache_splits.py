...
def main():
    parser = argparse.ArgumentParser(
        description="Check train/val/test splits in caches"
    )
    parser.add_argument(
        "--caches_dir",
        type=str,
        default="/home/ubuntu/model-east3/caches",
        help="Path to caches folder",
    )
    ...
    parser.add_argument(
        ...
        help="Base directory containing MoCo_seedXX folders",
    )
    ...
    for npz in sorted(npz_files):
        try:
            ...
            if not m:
                print(
                    f"[!] Skipping {os.path.basename(npz)} (Could not determine seed from path)"
                )
                continue
            ...
            if seed not in seed_to_splits:
                ...
                if not os.path.exists(train_csv):
                    print(
                        f"[!] Warning: CSV files not found for seed {seed} in {csv_dir}"
                    )
                    ...
                    print(
                        f"[✓] Loaded CSVs for seed {seed} (Train: {len(t_set)}, Val: {len(v_set)}, Test: {len(te_set)})"
                    )
            ...
            print("-" * 60)
            print(
                f"📂 {os.path.basename(os.path.dirname(npz))} / {os.path.basename(npz)}"
            )
            ...
            for split in ["train", "val", "test", "unknown"]:
                ...
                if split_total > 0:
                    details = ", ".join(
                        [
                            f"{CLASS_NAMES.get(k, str(k))}={counts[split][k]}"
                            for k in sorted(counts[split].keys())
                        ]
                    )
