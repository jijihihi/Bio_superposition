...
def main():
    parser = argparse.ArgumentParser(
        description="Check the progress of cache extraction."
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
    ...
    print("-" * 50)
    print(
        f"✅ 전체 진행률 : {total_extracted} / {target_total} 완료 ({(total_extracted/target_total)*100:.1f}%)"
    )
    print(
        f"   ▶ CNN 캐시: {total_cnn} / {target_cnn} 완료 ({(total_cnn/target_cnn)*100:.1f}%)"
    )
    print(
        f"   ▶ SAE 캐시: {total_sae} / {target_sae} 완료 ({(total_sae/target_sae)*100:.1f}%)"
    )
    print("=" * 50)
