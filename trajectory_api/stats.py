...
MUTATION_COLORS = {"SNCA": "#DD8452", "GBA": "#55A868", "LRRK2": "#C44E52", "Control": "#4C72B0"}
...
def plot_trajectory_statistics(
    ...
    # 2. Root Perturbation
    if n_root_perturb > 0:
        apop = adata_pair.obs["apoptosis"].values if "apoptosis" in adata_pair.obs else None
        if apop is not None:
            _run_root_perturbation(adata_pair, superclasses, apop, mutation, out_dir, prefix, dpi, n_root_perturb, n_dcs)
...
def _diagnose_dpt_distribution(dpt_vals, superclasses, mutation, out_dir, prefix, dpi):
    ...
    for grp, mask in [("Control", ctrl_mask), (mutation, mut_mask)]:
        ...
        logger.info(f"[{grp}] n={valid.sum()} | spread={spread:.4f} | mean={dpt_v.mean():.4f}")
        
        # Plot Histogram
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.hist(dpt_v, bins=60, alpha=0.7, color=MUTATION_COLORS.get(grp, "#888"), edgecolor="black", linewidth=0.3)
...
def _run_root_perturbation(adata, superclasses, apoptosis, mutation, out_dir, prefix, dpi, n_roots=10, n_dcs=10):
    ...
    if rhos:
        ...
        logger.info(f"Root Perturbation (n={len(rhos)}): Spearman mean={m:.4f}, std={s:.4f}")
        if s > 0.05:
