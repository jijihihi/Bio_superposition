...
def plot_dpt_feature_heatmap(
    ...
    fig.tight_layout()
    out_name = f"dpt_heatmap_{mutation}_{prefix}" if prefix else f"dpt_heatmap_{mutation}"
...
def plot_feature_trends(
    ...
    for f_idx in feature_indices:
        ...
        _plot_single_trend(
            dpt_vals, feat_vals, mutation, out_dir, f"{prefix}_feat{f_idx}", 
            dpi, gam_trim_pctl, alpha, point_size
        )

def _plot_single_trend(dpt_vals, feat_vals, mutation, out_dir, prefix, dpi, gam_trim_pctl, alpha, point_size):
    valid = np.isfinite(dpt_vals)
    ...
    fig.savefig(out_path.replace(".png", ".svg"), format="svg", bbox_inches="tight", transparent=True)
    plt.close(fig)
