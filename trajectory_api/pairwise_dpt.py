...
def _plot_diffmap(adata_pair, diffmap_dc, mutation, out_dir, prefix, dpi):
    ...
    # Control (gray)
    ax.scatter(x_vals[ctrl_mask], y_vals[ctrl_mask], c="#FFAB91", s=10, alpha=0.3, label="Control", rasterized=True)
    
    # Mutation (apoptosis gradient)
    if "apoptosis" in adata_pair.obs:
        apop = adata_pair.obs["apoptosis"].values[mut_mask]
        sc = ax.scatter(x_vals[mut_mask], y_vals[mut_mask], c=apop, cmap="YlOrRd", s=10, alpha=0.8, rasterized=True)
        plt.colorbar(sc, ax=ax, label="Apoptosis Rate")
    else:
        ax.scatter(x_vals[mut_mask], y_vals[mut_mask], c="#DD8452", s=10, alpha=0.8, label=mutation, rasterized=True)
    ...
    out_name = f"pairwise_diffmap_{mutation}_{prefix}" if prefix else f"pairwise_diffmap_{mutation}"
    fig.savefig(os.path.join(out_dir, f"{out_name}.png"), dpi=dpi, bbox_inches="tight")
    fig.savefig(os.path.join(out_dir, f"{out_name}.svg"), format="svg", bbox_inches="tight")
    plt.close(fig)
