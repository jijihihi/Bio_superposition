...
def save_ckpt(
    path, epoch, global_step, model, opt, scaler, xbm, best_acc, patience_counter, args
):
    ckpt = {
        "epoch": int(epoch),
        ...
        for epoch in range(self.start_epoch, self.args.epochs + 1):
            ...
            if is_eval_epoch:
                baked_encoder = bake_unitnorm_encoder(
                    self.model, self.args, self.device
                )
...
def get_args():
    p = argparse.ArgumentParser(
        "SupCon+XBM + Plate-balanced + LinearProbe ES (NO MoCo)"
    )
...
def main():
    ...
    args.symmetric_loss = (
        True  # BN 일관성 문제 해결됨 - 안정적 symmetric loss 사용 가능
    )
