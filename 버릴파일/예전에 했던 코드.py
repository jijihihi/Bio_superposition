...
from scipy.sparse.linalg import eigsh #대칭 행렬(Symmetric Matrix)에 한해서는 eigsh가 일반 eigs보다 정확하고 안정적. 큰거 몇개만 알면 될때 eigsh 유리.
...
class CNN4(nn.Module):
    def __init__(self):
        ...
        self.conv4 = weight_norm(nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1))
        self.relu4 = nn.ReLU()
...
class SimpleImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if label == 'control': label_idx = 0
        elif label == 'SNCA': label_idx = 1
        else: label_idx = 0

        try:
            image = Image.open(filepath).convert('RGB')
            return self.transform(image), torch.tensor(label_idx, dtype=torch.long), filepath
...
df_sampled = df_index_total.groupby("label", group_keys=False).apply(
...
transform = transforms.Compose([transforms.Resize((IMAGE_SIZE, IMAGE_SIZE)), transforms.ToTensor(), InstanceNormalize()])
...
if len(gap_feats) > 0:
    ...
    if len(df_sampled) != len(X_gap):
        raise ValueError(f"치명적 오류: 피처 개수({len(X_gap)})와 데이터프레임 개수({len(df_sampled)}) 불일치!")
...
ctrl_indices = np.where((df_sampled['label'] == 'control') | (df_sampled['label'] == 0))[0]
...
df_final = pd.merge(df_sampled, df_nmi[['reconstructed_path', 'NMI_Score']],
                    left_on='path', right_on='reconstructed_path', how='left')
...
for name, metric, norm_method in TEST_CONFIGS:
    ...
    if current_dim > N_COMPONENTS_PCA:
        print(f"   -> Applying PCA: Reducing dimensions from {current_dim} to {N_COMPONENTS_PCA}...")
        ...
        print(f"   -> Skipping PCA: Feature count ({current_dim}) <= {N_COMPONENTS_PCA}")
    ...
    try:
        if hasattr(L_mat, 'toarray'): L_mat = L_mat.toarray()
    ...
    # 제목에 R2 Score도 함께 표시
    plt.suptitle(f"Config: {name} | Est. Dim: {estimated_d:.2f} (Slope: {slope:.2f}, $R^2$: {r2_score:.3f})",
    ...
    if np.sum(valid_mask) > 3:
        ...
        # [수정] 라벨에 R^2 추가
        plt.plot(log_k, fit_line_y, 'r--', label=f'Fit (Slope={slope:.2f}, $R^2$={r2_score:.2f})')
        plt.xlabel("log(k)")
...
if not df_res.empty:
    ...
    print(df_res[['Metric', 'Est_Dim(d)', 'Max_Tau', 'Avg_Tau']].sort_values(by='Avg_Tau', ascending=False).to_string(index=False))
