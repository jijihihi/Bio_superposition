# ==============================================================================
# SupCon-variant (supervised contrastive) + MoCo momentum encoder (NO XBM)
...
#
# Key points:
# - q from model_q (train)
# - k from model_k (EMA teacher, eval, no grad)
# - No XBM / no extra memory bank
# - L2-normalize before projector and after projector
...
logger = logging.getLogger("SupConMoCo_Plate")
    ...
    def __init__(self, refs: List[SampleRef], ref_indices: List[int], img_size: int):
        ...
        for j, ridx in enumerate(tqdm(ref_indices, desc="preload", leave=True)):
            ...
            try:
                ...
                validate_uint16_rgb(img, self.img_size)
...
# ==============================================================================
# 7) MoCo momentum encoder (NO XBM)
# ==============================================================================
@torch.no_grad()
...
# ==============================================================================
# 9) Linear Probe (per-epoch, 3 epochs) + Early stopping
# ==============================================================================
@torch.no_grad()
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch in pbar:
            ...
            pbar.set_postfix({"lr": f"{lr:.2e}", "loss": f"{cur:.4f}"})
