...
def build_tar_index_if_needed(tar_path: str):
    ...
    pairs = [
        (p, it["tif_off"], it["tif_size"], it["js_off"], it["js_size"])
...
def load_all_sample_refs(shard_root: str) -> List[SampleRef]:
    ...
    if not tar_paths:
        raise FileNotFoundError(f"No tar shards under: {shard_root}")
    for tp in tar_paths:
...
class StrictPlateBalancedBatchSamplerOnBank(Sampler):
    def __init__(self, bank, batch_size, seed):
        ...
        for sup, line, plate in self.orig:
    ...
    def __len__(self):
        return max(1, sum(len(v) for v in self.orig.values()) // self.batch_size)
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for li, line in enumerate(self.control_lines):
                need = targets["Control"] // 3 + (
                    1 if li < targets["Control"] % 3 else 0
                )
                ...
                if plates:
                    ...
                    pp = need // len(plates)
                    for p in plates:
                        batch.extend(take("Control", line, p, pp))
                    for p in plates[: need % len(plates)]:
                        batch.extend(take("Control", line, p, 1))
            for sup in ["SNCA", "GBA", "LRRK2"]:
                ...
                if plates:
                    ...
                    for p in plates:
                        batch.extend(take(sup, sup, p, pp))
                    for p in plates[: targets[sup] % len(plates)]:
                        batch.extend(take(sup, sup, p, 1))
...
class InMemoryTarBank:
    def __init__(self, refs, ref_indices, img_size):
        ...
        n = len(ref_indices)
        self.images, self.labels, self.plates, self.lines, self.uids = (
            [None] * n,
            [0] * n,
            [""] * n,
            [""] * n,
        ...
        def read(tp, off, size):
            if tp not in tar_to_fh:
                tar_to_fh[tp] = open(tp, "rb", buffering=0)
            tar_to_fh[tp].seek(off)
            return tar_to_fh[tp].read(size)

        for j, ridx in enumerate(tqdm(ref_indices, desc="preload")):
            r = refs[ridx]
            try:
                img = tifffile.imread(
                    io.BytesIO(read(r.tar_path, r.tif_off, r.tif_size))
                )
                if img.dtype == np.uint16 and img.shape == (img_size, img_size, 3):
                    self.images[j] = img
                    self.labels[j], self.plates[j], self.lines[j] = (
                        r.label,
                        r.plate,
                        r.line,
                    )
...
class InMemorySixteenBitDataset(Dataset):
    def __init__(self, bank, indices, img_size, two_crops, augment):
        self.bank, self.ib, self.img_size, self.two_crops = (
            bank,
            indices,
            img_size,
            two_crops,
...
def save_split_csv(uids, labels, refs_by_uid, save_dir, filename):
    ...
    with open(os.path.join(save_dir, filename), "w", newline="", encoding="utf-8") as f:
        ...
        for uid, lb in zip(uids, labels):
            r = refs_by_uid[uid]
            w.writerow([uid, lb, r.superclass, r.line, r.plate, r.tar_path, r.prefix])
...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, dilation=1):
        ...
        self.proj = (
            conv2d(in_ch, out_ch, 1, 1, 0, bias=False) if in_ch != out_ch else None
        )
...
class Stage(nn.Module):
    def __init__(self, in_ch, out_ch, n_blocks, dilation, use_ckpt, ckpt_seg):
        ...
        self.blocks = nn.Sequential(
            ResBlock(in_ch, out_ch, dilation),
            *[ResBlock(out_ch, out_ch, dilation) for _ in range(n_blocks - 1)],
        )
    ...
    def forward(self, x):
        if (
            ...
            return checkpoint_sequential(
                self.blocks,
                min(self.ckpt_seg, len(self.blocks)),
                x,
                use_reentrant=False,
            )
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        return self.gap(self.trunk(x)).flatten(1)
...
class SupConMoCoModel(nn.Module):
    def __init__(
        ...
        self.projector = nn.Sequential(
            nn.Linear(OUT_DIM, proj_hidden, bias=False),
            nn.ReLU(),
...
def save_ckpt(
    path, epoch, step, model_q, model_k, opt, scaler, queue, best_acc, patience, args
):
    torch.save(
        {
            "epoch": epoch,
...
def load_ckpt(path, model_q, model_k, opt, scaler, queue, device):
    ...
    queue.keys.copy_(ckpt["queue"]["keys"].to(device))
    queue.labels.copy_(ckpt["queue"]["labels"].to(device))
...
def linear_probe_acc(args, device, encoder, train_loader, val_loader):
    ...
    if args.use_bf16:
        autocast_kw["dtype"] = torch.bfloat16
    ...
    with torch.inference_mode():
        for batch in val_loader:
            ...
            with torch.amp.autocast(**autocast_kw):
                feat = F.normalize(encoder(x), dim=1)
            correct += (probe(feat.float()).argmax(1) == y).sum().item()
            total += y.numel()
    del probe, opt
...
class Trainer:
    def __init__(
        ...
        self.args, self.model_q, self.model_k, self.queue = (
            args,
            model_q,
            model_k,
            queue,
        )
        self.train_loader, self.train_lp_loader, self.val_lp_loader = (
            train_loader,
            train_lp_loader,
            val_lp_loader,
        )
        ...
        self.start_epoch, self.global_step, self.best_acc, self.patience_counter = (
            1,
            0,
            -1,
            0,
        ...
        if args.auto_resume and os.path.exists(self.resume_path):
            ...
            logger.info(
                f"[resume] epoch={self.start_epoch}, step={gs}, best={ba*100:.2f}%"
            )
    ...
    def train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            with torch.no_grad():
                momentum_update_(self.model_q, self.model_k, self.args.moco_m)
            ...
            with torch.amp.autocast(**autocast_kw):
                ...
                if self.args.symmetric_loss:
                    ...
                    loss2 = supervised_contrastive_q_vs_k(
                        q2, y, all_k2, all_y, self.args.temp
                    )
                    loss = 0.5 * (loss + loss2)
            ...
            if self.use_bf16:
                ...
                if self.args.grad_clip > 0:
                    torch.nn.utils.clip_grad_norm_(
                        self.model_q.parameters(), self.args.grad_clip
                    )
                self.opt.step()
            else:
            ...
            pbar.set_postfix(
                {"lr": f"{lr:.2e}", "loss": f"{loss.item():.4f}", "queue": qsize}
            )
    ...
    def run(self):
        logger.info(
            f"Device: {self.device}, bf16={self.use_bf16}, queue_size={self.args.queue_size}"
        )
        ...
        for epoch in range(self.start_epoch, self.args.epochs + 1):
            ...
            val_acc = linear_probe_acc(
                self.args, self.device, enc, self.train_lp_loader, self.val_lp_loader
            )
            del enc
            torch.cuda.empty_cache()

            tqdm.write(
                f"Epoch {epoch} | Loss: {train_loss:.4f} | LP: {val_acc*100:.2f}%"
            )
            ...
            if val_acc > self.best_acc:
                ...
                tqdm.write(
                    f"  -> No improve ({self.patience_counter}/{self.args.patience})"
                )
...
def get_args():
    ...
    p.add_argument(
        "--save_dir",
        type=str,
        default="/content/drive/MyDrive/Final_paper/Model_MoCo_Queue",
    )
    ...
    # Queue
    p.add_argument(
        "--queue_size", type=int, default=65536, help="Queue capacity (e.g., 65536)"
    )
    p.add_argument(
        "--queue_start_epoch",
        type=int,
        default=2,
        help="Epoch to start using queue for loss",
    )
    # LP
...
def main():
    ...
    train_csv, val_csv, test_csv = [
        os.path.join(args.save_dir, f"{s}_split.csv") for s in ["train", "val", "test"]
    ]
    ...
    if will_resume and all(os.path.exists(p) for p in [train_csv, val_csv, test_csv]):
        ...
        X_temp, X_test, y_temp, _, st_temp, _ = train_test_split(
            uids,
            labels,
            strat,
        ...
        save_split_csv(
            X_test,
            [refs_by_uid[u].label for u in X_test],
            refs_by_uid,
            args.save_dir,
            "test_split.csv",
        )
    ...
    train_ds = InMemorySixteenBitDataset(
        train_bank, list(range(len(train_refidx))), args.img_size, True, True
    )
    train_lp_ds = InMemorySixteenBitDataset(
        train_bank, list(range(len(train_refidx))), args.img_size, False, False
    )
    val_lp_ds = InMemorySixteenBitDataset(
        val_bank, list(range(len(val_refidx))), args.img_size, False, False
    )

    sampler = StrictPlateBalancedBatchSamplerOnBank(
        train_bank, args.batch_size, args.seed
    )
