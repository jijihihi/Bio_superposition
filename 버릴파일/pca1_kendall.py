...
def get_args():
    ...
    # PCA sweep
    p.add_argument("--max_pc", type=int, default=20,
                   help="Sweep PC1..PCk, report tau for each")
    p.add_argument("--n_pca", type=int, default=50,
                   help="Total PCA components to compute")
...
def plot_pc_tau_sweep(
    ...
    ax.text(0.98, 0.95, f"Best: PC{best_idx+1} (τ={pc_taus[best_idx]:.4f})",
...
def plot_pc1_scatter(
    ...
    ax.text(0.95, 0.05, f"n = {len(pc1_mut)}\nτ = {tau:.4f}",
...
def main():
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"PC-score analysis (norm={args.norm}, max_pc={args.max_pc}, "
    ...
    for mut in mutations:
        ...
        if has_de:
            ...
            if args.de_top_k > 0 and mask.sum() > args.de_top_k:
                ...
                mask[top_k_idx] = True
                logger.info(f"    DE top-{args.de_top_k}: "
        ...
        logger.info(f"    PCA: {X_sub.shape[1]}D → {n_pca}D, "
        ...
        logger.info(f"    Control medoid: idx={medoid_idx}, "
        ...
        for k in range(min(10, max_pc)):
            ...
            logger.info(f"      PC{k+1}: τ={pc_taus[k]:+.4f} "
                         f"(p={pc_pvals[k]:.2e}, "
        ...
        sweep_path = os.path.join(out_dir,
            f"pc_sweep_{mut}_{which_layer}.png")
        plot_pc_tau_sweep(
            mut, pc_taus, sweep_path,
        ...
        scatter_path = os.path.join(out_dir,
            f"pc1_scatter_{mut}_{which_layer}.png")
        plot_pc1_scatter(
            pc1_score[valid], apop_mut[valid],
            mut, pc1_tau, scatter_path, dpi=args.dpi,
        )
        ...
        if best_k != 0:
            ...
            best_path = os.path.join(out_dir,
                f"pc{best_k+1}_scatter_{mut}_{which_layer}.png")
            plot_pc1_scatter(
                best_score[valid], apop_mut[valid],
                mut, best_tau, best_path, dpi=args.dpi,
            )
