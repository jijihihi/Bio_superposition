# ==============================================================================
# SupCon-variant (supervised contrastive) + MoCo momentum encoder + Supervised XBM
...
class SupConMoCoModel(nn.Module):
    ...
    def forward(self, x, return_proj_for_cov: bool = False):
        ...
        if return_proj_for_cov:
            # Return both: z_norm for SupCon, z (before L2 norm) for covariance loss
            # VICReg applies covariance loss on projector output before normalization
            return z_norm, z
        return z_norm


# ==============================================================================
# 7) MoCo momentum encoder + Supervised XBM
# ==============================================================================
@torch.no_grad()
...
@torch.no_grad()
def bake_unitnorm_encoder(model_q: SupConMoCoModel, args, device):
    ...
    for ep in range(1, args.lp_epochs + 1):
        ...
        for batch in pbar:
            ...
            for i in range(0, x_cpu.size(0), enc_bs):
                ...
                feat = feat.detach().to(dtype=probe_dtype).requires_grad_(True)
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        self.model_q.train()
        self.model_k.eval()  # momentum encoder는 항상 eval 모드 (BN/Dropout 비활성화)
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast(**autocast_kwargs):
                # Get projector output (z) for covariance loss, z_norm for SupCon
                q, proj_v1 = self.model_q(v1, return_proj_for_cov=True)
                with torch.no_grad():
                    k = self.model_k(v2)
                ...
                # Covariance loss on projector output (VICReg-style)
                loss_cov = covariance_loss(proj_v1)

                if self.args.symmetric_loss:
                    q2, proj_v2 = self.model_q(v2, return_proj_for_cov=True)
                    with torch.no_grad():
                        k2 = self.model_k(v1)
                    ...
                    # Average covariance loss from both views (projector output)
                    loss_cov = 0.5 * (loss_cov + covariance_loss(proj_v2))

                # Total loss: SupCon + weighted covariance loss
...
# ==============================================================================
def get_args():
    p = argparse.ArgumentParser("SupCon+MoCo+XBM + Plate-balanced + LinearProbe ES")

    # Experiment
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--save_dir", type=str, default="/content/drive/MyDrive/Final_paper/Model_MoCoXBM_PlateLP_LRRK2_L2norm_hidden2048_resume_bias=True_clean_image_cov_loss")
    ...
    # Model
    p.add_argument("--embed_dim", type=int, default=2048,
                   help="Projector output dim. Larger (2048) for VICReg-style decorrelation.")
    ...
    p.add_argument("--cov_coeff", type=float, default=0.04,
                   help="weight for covariance loss (VICReg-style). prevents feature collapse.")
    ...
    p.add_argument("--lp_eval_interval", type=int, default=10,
...
def main():
    ...
    args.symmetric_loss = True # symmetric loss 사용하는것.
