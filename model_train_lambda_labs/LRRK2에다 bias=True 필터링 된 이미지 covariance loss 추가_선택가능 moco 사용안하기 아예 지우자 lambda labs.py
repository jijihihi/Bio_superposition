# ==============================================================================
# SupCon-variant (supervised contrastive) + Supervised XBM (NO MoCo)
# + Plate/Line balanced batching (strict per-plate equal allocation within each line)
# + Per-epoch Linear Probe (3 epochs) for Early Stopping
#
# 양 보정 위해서 L2 정규화해서 projector 전에 넣어주고, projector 나온 후에도 양 보정 L2 정규화로 해준다.
# csv 저장 코드도 추가.
#
# Data source: plate-wise tar shards created earlier
#   /content/wds_shards/<LINE>/plate=005005/<LINE>_plate=005005-000000.tar
# Each tar contains:
#   <key>.tif  (raw tif bytes, unchanged)
...
logger = logging.getLogger("MoCoXBM_Plate")
...
def select_balanced_subset(refs: List[SampleRef], max_samples_total: int, seed: int) -> List[int]:
    ...
    - Control은 3 line(C4,C18,C19)로 균등 분배
    - 각 line 내부는 plate별로 가능한 균등(= plate마다 비슷한 수)
    """
    ...
    # Control split across 3 lines 같으 control이더라도 라인별로 비슷한 개수 이미지 들어가도록. control line이 다르더라고 control의 본질적인 특성 학습하도록.
    ...
    # Mutation lines = one line each (SNCA/GBA/LRRK2)
...
# ==============================================================================
# 4) Strict plate-uniform batch sampler
#     - per batch: allocate per superclass then per Control line
#     - within each line: allocate per plate = floor(need / num_plates) + remainder
# ==============================================================================
...
# ==============================================================================
# 5) Dataset: preload tar -> RAM (uint16), reuse same images for probe
# ==============================================================================
...
# instance normalization을 해서 이미지마다 다르게 찍힌 초점이나 밝기 등 보정해서 shortcut learning을 하지 못하도록 하려고. 이때 특정 소기관이 찍히지 않은 이미지가 있을 수 있다. 이때 그 채널의 표준편차는 매우 작을 것이므로
### 그 작은 표준편차로 나눠주면 그 채널 값들이 매우 커질 수 있다. 따라서 세포 소기관이 없는경우 표준편차가 작을테니까 0.01을 기준으로 해서 나눠줌.
...
def build_projector(in_dim: int, embed_dim: int, proj_hidden: int) -> nn.Module:
    """
    2-layer Projector MLP for contrastive learning.
    - bias=False (standard for contrastive)
    - No BN, No Dropout (simplest, most stable)
    """
...
def covariance_loss(z: torch.Tensor) -> torch.Tensor:
    # covariance loss에서 각 특징들간의 내적으로 각 특징이 연관성이 있는지 없는지를 판단한다.
    # 근데 내적으로 계산하는거 자체가 공간이 평평하다고 가정하는것이다.
    # 근데 manifold hypothesis에 따라 데이터는 평평하게 존재할 확률은 낮다. 즉 각 특징들의 관계가 없거나 혹은 선형적인 관계만을 가질 가능성은 극히 낮다.
    # 그러나 covariance loss를 주기 때문에. 데이터를 저장하는 방법은 A B 저장할 수도 있지만 f(A,B), g(A,B)와 같은 방법으로 저장 할수도 있따
    # 예를 들어 A+B /2 A-B/2 와 같은 방식. 이렇게 저장하는 방식은 훨씬 많을 것이다.
    # 인공지능은 supcon의 loss를 줄이면서 covariance loss도 줄여야하기때문에 정보의 손실 없이 f g 함수를 배워서 f와 g가 서로 수직으로 하게끔 정렬해야한다.
    # 즉 선형적 관계를 가진다고 가정한다기 보다 covariance loss가 선형적인 represenstation을 하도록 "강제"한다
    # 트랜스포머에서도 쿼리 키 내적하는 방식으로 유사도 측정하는데 지금 논의와 동일하다.
    # 즉 수학적 불완전함 등이 (수학적 무지 혹은 계산량 때문에) 오히려 딥러닝의 발전이나 해석 가능성을 이끌 었을 가능성이 있다.
    # 즉 선형성이 "데이터의 본질"이라서가 아니라, 우리가 준 자(Ruler)가 직선 자밖에 없어서, 모델이 모든 것을 직선으로 펴버린 것"
    # N차원 공간에 N개 이상의 특징을 구겨 넣으려면, 비선형적으로 꼬아 넣을 수도 있다.
...
def save_ckpt(path, epoch, global_step, model, opt, scaler, xbm,
...
def load_ckpt(path, model, opt, scaler, xbm, device, strict=False):
    # PyTorch 2.6+: weights_only=False 필요 (numpy RNG state 등 포함)
    ckpt = torch.load(path, map_location="cpu", weights_only=False)

    # Support both old (model_q/model_k) and new (model) checkpoint formats
    if "model" in ckpt:
        model.load_state_dict(ckpt["model"], strict=strict)
    else:
        model.load_state_dict(ckpt["model_q"], strict=strict)
    ...
    # XBM restore
    xbm.reset()
    xbm.feats.copy_(ckpt["xbm"]["feats"].to(device=device, dtype=xbm.dtype))
    xbm.labels.copy_(ckpt["xbm"]["labels"].to(device=device))
    xbm.ptr = int(ckpt["xbm"]["ptr"])
    xbm.full = bool(ckpt["xbm"]["full"])
...
# ==============================================================================
# 8) Linear Probe (per-epoch, 3 epochs) + Early stopping
# ==============================================================================
# encoder freeze하고 linear probe
@torch.no_grad()
def bake_unitnorm_encoder(model: nn.Module, args, device):
    ...
    for ep in range(1, args.lp_epochs + 1):
        ...
        for batch in pbar:
            ...
            for i in range(0, x_cpu.size(0), enc_bs):
                ...
                # Detach and convert to probe dtype, then enable grad for probe
                feat = feat.detach().to(dtype=probe_dtype)
...
# ==============================================================================
class Trainer:
    def __init__(self, args, model, xbm, train_loader, train_lp_loader, val_lp_loader):
        self.args = args
        self.model = model
        self.xbm = xbm
        ...
        self.opt = optim.AdamW(self.model.parameters(), lr=args.lr, weight_decay=args.wd)
        ...
        # ✅ resume checkpoint는 별도로 저장 (학습 재개용)
        self.resume_ckpt_path = os.path.join(args.save_dir, "resume_ckpt.pt")

        renorm_unit_per_out_channel_(self.model.encoder)  # encoder만 renorm (GAP 안정성)
        ...
        if resume_path and os.path.exists(resume_path):
            ...
            ep, gs, best_acc, pat = load_ckpt(
                ...
                strict=False  # 항상 non-strict (호환성 유지)
            )
    ...
    def _maybe_xbm_reset(self, epoch: int):
        if epoch == int(self.args.xbm_start_fill_epoch):
            logger.info("[XBM] reset at fill-only epoch")
            self.xbm.reset()
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast(**autocast_kwargs):
                # ============================================================
                # Step 1: All gradient-enabled forwards (train mode)
                # - BN running stats are updated consistently in this phase
                # ============================================================
                q, gap_v1 = self.model(v1, return_gap_for_cov=True)
                
                if self.args.symmetric_loss:
                    # q2 also needs gradient, compute in same train mode phase
                    q2, gap_v2 = self.model(v2, return_gap_for_cov=True)
                
                # ============================================================
                # Step 2: All no-gradient forwards (train mode 유지)
                # - eval/train 전환 없이 gradient만 차단
                # - BN running stats는 계속 업데이트됨
                # ============================================================
                with torch.no_grad():
                    k = self.model(v2)
                    if self.args.symmetric_loss:
                ...
                # ============================================================
                # Step 3: Build key pools and compute losses
                # ============================================================
                # Primary loss: q (from v1) vs k (from v2) + XBM
                k_list = [k]
                y_list = [y]
                if use_xbm:
                    qbm, ybm = self.xbm.get()
                    if qbm.numel() > 0:
                        k_list.append(qbm)
                        y_list.append(ybm)
                ...
                # Covariance loss on GAP features
                loss_cov = covariance_loss(gap_v1)

                # Symmetric loss: add q2 (from v2) vs k2 (from v1) + XBM
                if self.args.symmetric_loss:
                    k_list2 = [k2]
                    y_list2 = [y]
                    if use_xbm:
                        # Note: qbm, ybm already fetched above, reuse
                        if qbm.numel() > 0:
                            k_list2.append(qbm)
                            y_list2.append(ybm)
                    ...
                    loss_cov = 0.5 * (loss_cov + covariance_loss(gap_v2))

                # Total loss
                if self.args.use_cov_loss:
                    loss = loss_supcon + float(self.args.cov_coeff) * loss_cov
                else:
                    loss = loss_supcon
            ...
            if self.use_bf16:
                ...
                if self.args.grad_clip > 0:
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), float(self.args.grad_clip))
                self.opt.step()
            else:
            ...
            if (self.global_step % int(self.args.renorm_every)) == 0:
                renorm_unit_per_out_channel_(self.model.encoder)  # encoder만 renorm (GAP 안정성)

            if fill_xbm:
                with torch.no_grad():
                    self.xbm.enqueue(k.detach(), y)

            cur = float(loss.item())
            cur_cov = float(loss_cov.item())
            total_loss += cur
            steps += 1
            self.global_step += 1
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs + 1):
            ...
            if is_eval_epoch:
                baked_encoder = bake_unitnorm_encoder(self.model, self.args, self.device)
            ...
            if is_eval_epoch and val_acc > self.best_acc:
...
# ==============================================================================
# 11) Args (one place to edit)
# ==============================================================================
def get_args():
    p = argparse.ArgumentParser("SupCon+XBM + Plate-balanced + LinearProbe ES (NO MoCo)")
    ...
    # 모델 좀 크게하니까 성능 증가. 생각보다 배치 사이즈가 병목이 아니었음.
    ...
    p.add_argument("--proj_hidden", type=int, default=2048,
                  help="hidden dim for 2-layer projector MLP")
...
# ==============================================================================
# 12) Main: refs -> subset -> split -> bank -> datasets/loaders -> train
# ==============================================================================
def main():
    args = get_args()
    args.use_bf16 = True  # Colab/A100 권장
    args.symmetric_loss = True  # BN 일관성 문제 해결됨 - 안정적 symmetric loss 사용 가능
    ...
    # sampler (strict plate-balanced for training)
    ...
    trainer = Trainer(args, model, xbm, train_loader, train_lp_loader, val_lp_loader)
    trainer.run()
