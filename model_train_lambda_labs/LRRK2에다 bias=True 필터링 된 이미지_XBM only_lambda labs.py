...
#
# Lambda Labs 환경용 (파일 시스템: model-east3)
# MoCo 제거 - XBM만 사용
# ==============================================================================
...
logger = logging.getLogger("XBM_Only")


# ==============================================================================
# 0) Dataset layout - Lambda Labs
# ==============================================================================
...
def save_ckpt(path, epoch, global_step, model, opt, scaler, xbm, best_acc, patience_counter, args):
    ckpt = {
        "epoch": int(epoch),
...
class Trainer:
    def __init__(self, args, model, xbm, train_loader, train_lp_loader, val_lp_loader):
        ...
        renorm_unit_per_out_channel_(self.model.encoder)
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast(**autocast_kwargs):
                # Forward q (gradient enabled)
                q = self.model(v1)
                
                if self.args.symmetric_loss:
                    q2 = self.model(v2)
                
                # Forward k (no gradient, eval mode for BN consistency)
                self.model.eval()
                with torch.no_grad():
                    k = self.model(v2)
                    if self.args.symmetric_loss:
...
# ==============================================================================
def get_args():
    p = argparse.ArgumentParser("SupCon+XBM (NO MoCo) - Lambda Labs")

    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--save_dir", type=str, default="/home/ubuntu/model-east3/outputs/Model_XBM_only")
    ...
    p.add_argument("--xbm_start_fill_epoch", type=int, default=3)
    p.add_argument("--xbm_start_use_epoch", type=int, default=4)
