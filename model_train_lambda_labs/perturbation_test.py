...
class InMemorySixteenBitDataset(Dataset):
    ...
    def __init__(
        self, bank: InMemoryTarBank, indices_in_bank: List[int], img_size: int
    ):
        self.bank = bank
...
def grid_shuffle_batch_gpu(images: torch.Tensor, grid_size: int) -> torch.Tensor:
    ...
    # 재조합
    # (B, C, grid_size, grid_size, patch_h, patch_w)
    shuffled_patches = shuffled_patches.view(
        B, C, grid_size, grid_size, patch_h, patch_w
    )
    # (B, C, grid_size, patch_h, grid_size, patch_w)
...
def grid_shuffle_batch_gpu_non_jit(
    images: torch.Tensor, grid_size: int
) -> torch.Tensor:
    """
    Non-JIT 버전 (fallback)
    """
...
def visualize_shuffled_examples(
    ...
    for i, idx in enumerate(indices[:num_examples]):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(save_dir, f"example_{i+1}_{class_name}.png"),
            dpi=100,
            bbox_inches="tight",
        )
        plt.close()
...
def run_perturbation_test(args):
    ...
    if isinstance(checkpoint, dict):
        if "model_q" in checkpoint:
            ...
            encoder_state = {
                k.replace("_orig_mod.", ""): v for k, v in encoder_state.items()
            }
        else:
    ...
    logger.info(
        f"Train features shape: {train_features_gpu.shape}, device: {train_features_gpu.device}"
    )
    ...
    report = classification_report(
        test_labels_np, baseline_preds, target_names=class_names
    )
    logger.info(f"\nClassification Report:\n{report}")
...
def get_args():
    p = argparse.ArgumentParser(
        "Perturbation Test: Grid Shuffle Evaluation (A100 Optimized)"
    )
    ...
    p.add_argument(
        "--output_dir",
        type=str,
        default="/content/perturbation_results",
    ...
    p.add_argument(
        "--batch_size", type=int, default=512, help="Batch size (larger for A100)"
    )
    ...
    p.add_argument(
        "--lp_momentum", type=float, default=0.9, help="Linear probe momentum"
    )
    p.add_argument(
        "--lp_batch_size", type=int, default=8192, help="Linear probe batch size"
    )

    # GPU settings (A100 optimized)
    p.add_argument("--device", type=str, default="cuda", help="Device")
    p.add_argument(
        "--use_bf16", action="store_true", default=True, help="Use BF16 (A100)"
    )
    p.add_argument(
        "--use_compile", action="store_true", default=False, help="Use torch.compile"
    )
