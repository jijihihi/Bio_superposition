...
class Stage(nn.Module):
    def __init__(self, in_ch, out_ch, n_blocks, dilation, use_ckpt: bool = False, ckpt_segments: int = 1):
        super().__init__()
                        ...
                        for prefix in prefixes:
                            ...
                            for mem in members:
                                if mem.name == tif_name:
                                    refs_by_class[label].append(SampleRef(
                                        tar_path=tar_path,
                                        prefix=prefix,
...
def main():
    ...
    with open(stats_csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        header = ["channel", "mean", "std", "CV (std/|mean|)"] + list(class_means.keys())
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Dead channels (std < {dead_threshold}): {dead_count}/{OUT_DIM} ({dead_count/OUT_DIM*100:.1f}%)\n")
        f.write(f"Low CV channels (CV < {low_cv_threshold}): {low_cv_count}/{OUT_DIM} ({low_cv_count/OUT_DIM*100:.1f}%)\n\n")
    ...
    ax.axhline(dead_threshold, color='r', linestyle='--', label=f'Dead threshold ({dead_threshold})')
    ...
    ax.axvline(low_cv_threshold, color='r', linestyle='--', label=f'Low CV threshold ({low_cv_threshold})')
    ...
    ax = axes[1, 0]
    available_classes = [cls for cls in ["Control", "SNCA", "GBA", "LRRK2"] if cls in class_means]
    if len(available_classes) > 0:
        class_mean_matrix = torch.stack([class_means[cls] for cls in available_classes], dim=0)
        im = ax.imshow(class_mean_matrix.numpy(), aspect='auto', cmap='RdBu_r', vmin=-0.2, vmax=0.2)
    ...
    if "Control" in class_means and len(available_classes) > 1:
        ...
        im = ax.imshow(diff_matrix.numpy(), aspect='auto', cmap='RdBu_r', vmin=-0.1, vmax=0.1)
    ...
    print(f"   Low CV channels (CV < {low_cv_threshold}): {low_cv_count}/{OUT_DIM} ({low_cv_count/OUT_DIM*100:.1f}%)")
