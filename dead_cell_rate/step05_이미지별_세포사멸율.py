...
def main():
    ...
    for folder_name in folder_list:
        ...
        for mask_file in tqdm(mask_files, desc=f"   {folder_name}", leave=True, ncols=100):
            try:
                # cytox 파일 찾기
                cytox_filename = mask_file.name.replace("_mask.tif", "_cytox.tif").replace("_mask.tiff", "_cytox.tiff")
                ...
                intensity_rate = apoptotic_intensity / total_intensity if total_intensity > 0 else 0.0
    ...
    for cls in ['Control', 'GBA', 'LRRK2', 'SNCA']:
        if cls in class_stats:
            ...
            print(f"{cls:<12} {len(rates):<8} {np.mean(rates):.6f}   {np.std(rates):.6f}   {np.min(rates):.6f}   {np.max(rates):.6f}")
