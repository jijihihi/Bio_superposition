...
def get_args():
    ...
    parser.add_argument(
        "--qc_filter", type=str, choices=['all', 'qc_only', 'both'],
        default='both',
...
def analyze_results(df, qc_filter_value):
    ...
    # 알고리즘 목록 추출
    rate_cols = [c for c in df.columns if c.startswith('rate_') and not c.startswith('rate_mean')]
    intensity_rate_cols = [c for c in df.columns if c.startswith('intensity_rate_')]
    intensity_per_pixel_cols = [c for c in df.columns if c.startswith('intensity_per_pixel_')]
    ...
    for radius in radii:
        ...
        for algo in algorithms:
            for metric_type in ['rate', 'intensity_rate', 'intensity_per_pixel']:
                ...
                if len(control_row) == 0 or len(gba_row) == 0 or len(lrrk2_row) == 0 or len(snca_row) == 0:
                    continue
                ...
                if gba_val > control_val and lrrk2_val > control_val and snca_val > control_val:
                    # threshold 값 추출
                    thresh_col = f'global_thresh_{algo}'
                    thresh_val = control_row[thresh_col].values[0] if thresh_col in df_radius.columns else np.nan
                    ...
                    valid_combinations.append({
                        ...
                        'LRRK2': lrrk2_val,
                        'SNCA': snca_val,
                        'GBA_vs_Control': gba_val / control_val if control_val > 0 else np.inf,
                        'LRRK2_vs_Control': lrrk2_val / control_val if control_val > 0 else np.inf,
                        'SNCA_vs_Control': snca_val / control_val if control_val > 0 else np.inf,
                        'min_ratio': min(gba_val / control_val, lrrk2_val / control_val, snca_val / control_val) if control_val > 0 else np.inf,
                    })
...
def main():
    ...
    print("=" * 70)
    print(f"{'QC':<10} {'Radius':<8} {'Algorithm':<12} {'Metric':<20} {'MinRatio':<10} {'Control':<12} {'GBA':<12} {'LRRK2':<12} {'SNCA':<12}")
    print("-" * 110)
    
    for _, row in df_valid.head(10).iterrows():
        print(f"{row['qc_filter']:<10} {row['rolling_ball_radius']:<8} {row['algorithm']:<12} {row['metric_type']:<20} {row['min_ratio']:.4f}    {row['Control']:.6f}   {row['GBA']:.6f}   {row['LRRK2']:.6f}   {row['SNCA']:.6f}")
