...
def get_args():
    ...
    parser.add_argument(
        "--total_samples", type=int, default=28800,
    ...
    parser.add_argument(
        "--rolling_ball_radii", type=int, nargs='+',
        default=[25]
    )
    ...
    parser.add_argument("--min_nucleus_fraction", type=float, default=0.25) #전체 픽셀 pooling 즉 핵과 cytoxgreen pooling해서 threshold 잡고 세포사멸율 확인하는거니까 핵 비율 fraction할 필요 없지
...
def calculate_global_thresholds(all_cytox_intensity, radius_label=""):
    ...
    # Basic methods
    # for name, func in [('otsu', threshold_otsu), ('li', threshold_li), 
    #                    ('yen', threshold_yen), ('isodata', threshold_isodata),
    #                    ('mean', threshold_mean), ('triangle', threshold_triangle)]:
    #     try:
    ...
    print("\n   [Threshold 비교: 낮음 → 높음]")
    sorted_thresh = sorted([(k, v) for k, v in thresholds.items() if not np.isnan(v)], key=lambda x: x[1])
...
def get_folder_list():
    return ["Control_C4", "Control_C18", "Control_GBA_C19", "Control_SNCA_C19", 
            "SNCA", "GBA", "LRRK2"]


def get_sampling_plan(total_samples):
    base_ratio = {
        "Control_C4": 2400, "Control_C18": 2400, 
        "Control_GBA_C19": 1200, "Control_SNCA_C19": 1200,
        "SNCA": 7200, "GBA": 7200, "LRRK2": 7200,
    }
...
def load_images_from_folder(folder_path, args, is_qc=True):
    ...
    for mask_file in mask_files:
        try:
            cytox_file = mask_file.parent.parent / "cytoxgreen" / mask_file.name.replace("_mask", "_cytox")
            ...
            for cls in ['Control', 'GBA', 'LRRK2', 'SNCA']:
                ...
                for algo, thresh in global_thresholds.items():
                    ...
                    if np.isnan(thresh):
                        ...
                        result[f'intensity_rate_{algo}'] = apop_intensity / total_intensity if total_intensity > 0 else np.nan
                        result[f'intensity_per_pixel_{algo}'] = apop_intensity / total_pixels
