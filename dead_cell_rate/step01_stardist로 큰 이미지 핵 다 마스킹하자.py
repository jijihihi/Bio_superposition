...
def segment_nuclei(args):
    ...
    # Load StarDist model
    print(
        f"--- 🤖 StarDist 모델 로드 중 (prob={args.prob_thresh}, nms={args.nms_thresh}) ---"
    )
    ...
    for folder_name in args.folders:
        ...
        for tif_path in tif_files:
            ...
            try:
                ...
                if data_cube.ndim == 2:
                    ...
                    if data_cube.shape[2] <= 4:  # Likely (H, W, C)
                        if data_cube.shape[2] <= args.nuc_channel:
                            print(
                                f"  - ⚠️ '{tif_path.name}' 파일에서 핵 채널({args.nuc_channel})을 찾을 수 없어 건너뜁니다."
                            )
                            continue
                    ...
                    print(
                        f"  - ⚠️ '{tif_path.name}' 지원하지 않는 이미지 차원: {data_cube.shape}"
                    )
                    continue
                ...
                print(
                    f"  - {tif_path.name}: 총 {total_nuclei}개 -> 유효 핵: {valid_nuclei_count}개 "
                ...
                if args.save_visualization:
                    try:
                        nuc_ch_8bit = rescale_intensity(
                            nuc_ch_data, in_range="image", out_range=(0, 255)
                        ).astype(np.uint8)
                        ...
                        output_vis_path = (
                            vis_output_dir / f"{base_filename}_visualization.png"
                        )
