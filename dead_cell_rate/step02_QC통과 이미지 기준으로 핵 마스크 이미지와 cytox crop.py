        ...
        for qc_file in tqdm(qc_files, desc=f"   {folder_name}", leave=False):
            ...
            if mask_img is not None:
                try:
                    ...
                    if (
                        cropped_mask.shape[0] == args.patch_size
                        and cropped_mask.shape[1] == args.patch_size
                    ):
            ...
            if plate_position not in cytox_cache:
                cytox_file = find_cytox_file(
                    args.cytox_dir, folder_name, plate_position
                )
                if cytox_file:
                    try:
            ...
            if cytox_img is not None:
                try:
                    ...
                    if (
                        cropped_cytox.shape[0] == args.patch_size
                        and cropped_cytox.shape[1] == args.patch_size
                    ):
