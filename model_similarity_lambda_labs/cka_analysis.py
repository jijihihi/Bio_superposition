...
from sae_project.step02_logging_utils import (CLASS_TO_LABEL,
                                              DEFAULT_SHARD_ROOT, OUT_DIM,
                                              get_logger)
from sae_project.step03_data_shards import (SampleRef, build_uid_to_refidx,
...
def extract_features(
    model: SupConMoCoModelForCKA, dataloader: DataLoader, device: torch.device
...
def run_cka_analysis(args):
    ...
    for class_label, class_name in enumerate(classes):
        ...
        if mask.sum() > 10:
            ...
            logger.info(
                f"  {class_name}: Linear CKA = {cka_class:.4f} (n={mask.sum()})"
            )
        else:
    ...
    if linear_cka >= 0.9:
        ...
        print(
            "  → The two models have MODERATELY SIMILAR representations (0.5 ≤ CKA < 0.7)"
        )
    else:
