...
def _to_numpy(t: torch.Tensor) -> np.ndarray:
    ...
    try:
        ...
        return (
            np.frombuffer(bytes(t.untyped_storage()), dtype=np_dtype)
            .reshape(t.shape)
            .copy()
        )
...
def load_encoder_for_cka(ckpt_path: str, args) -> EncoderWithCKAHook:
    ...
    if missing:
        ...
        real_missing = [
            k for k in missing if not any(k.startswith(e) for e in expected_missing)
        ]
        if real_missing:
...
def extract_features(
    encoder: EncoderWithCKAHook, dataloader: DataLoader, device: torch.device
...
# ==============================================================================
def sample_balanced_per_class(
    refs: List[SampleRef], ref_indices: List[int], num_samples_per_class: int, seed: int
) -> List[int]:
...
def run_cka_on_caches(args):
    ...
    for i, path in enumerate(args.caches):
        ...
        if "X_all" in data:
            ...
            if "usage_ema" in data:
                ...
                X = X[:, alive_mask]
                logger.info(
                    f"[{i+1}] {path}: alive SAE neurons: {alive_mask.sum()}/{len(alive_mask)}"
                )
        elif "X_gap" in data:
            X = data["X_gap"]
        ...
        if all_labels is None:
            ...
            logger.info(
                f"Subsampled {len(sub_indices)} images for CKA (up to {args.samples_per_class} per class for classes {target_classes})."
            )

        else:
            assert np.array_equal(
                y[sub_indices], all_labels
            ), f"Label mismatch in {path}!"
    ...
    def compute_and_save_cka(features_list, filename_suffix, title):
        ...
        csv_path = os.path.join(
            args.output_dir, f"cka_matrix_caches_{filename_suffix}.csv"
        )
        with open(csv_path, "w", newline="") as f:
            ...
            for i, name in enumerate(model_names):
                writer.writerow(
                    [name] + [f"{cka_matrix[i, j]:.4f}" for j in range(num_models)]
                )
        logger.info(f"Saved CKA matrix to: {csv_path}")
        return cka_matrix

    # 1. Global CKA
    global_cka = compute_and_save_cka(
        all_features, "global", "Computing Global CKA Matrix (Classes 0,1,2,3)"
    )
    ...
    for cls in target_classes:
        ...
        if mask.sum() > 0:
            ...
            compute_and_save_cka(
                cls_features,
                f"class_{cls_name}",
                f"Computing CKA Matrix for Class: {cls_name} (n={mask.sum()})",
            )
...
def run_cka_analysis(args):
    ...
    if len(all_eval_uids) > 0:
        ...
        eval_ref_indices = [
            uid_to_ref_idx[uid] for uid in unique_uids if uid in uid_to_ref_idx
        ]
        logger.info(
            f"Total evaluation images: {len(eval_ref_indices)} (val + test combined)"
        )
        ...
        if args.num_samples > 0:
            ...
            logger.info(
                f"Sampling {num_per_class} images per class (total: {num_per_class * num_classes})"
            )
            ref_indices = sample_balanced_per_class(
                refs, eval_ref_indices, num_per_class, args.seed
            )
        else:
            ref_indices = eval_ref_indices
    else:
        logger.info(
            f"No val_split.csv or test_split.csv found. Sampling from all images..."
        )
        ...
        ref_indices = sample_balanced_per_class(
            refs, all_indices, num_per_class, args.seed
        )
    ...
    for i, ckpt_path in enumerate(ckpt_paths):
        ...
        stage5_feat, refine_gap_feat, labels, uids = extract_features(
    ...
    if num_models == 2:
        ...
        for class_label, class_name in enumerate(classes):
            ...
            if mask.sum() > 10:
                ...
                logger.info(
                    f"  {class_name}: stage5={cka_s5:.4f}, refine_gap={cka_gap:.4f} (n={mask.sum()})"
                )
    ...
    results = {
        ...
        "per_class_linear_cka_refine_gap": (
            class_cka_gap_results if class_cka_gap_results else None
        ),
    }
    ...
    for tag, mat in [("stage5", cka_matrix), ("refine_gap", cka_matrix_gap)]:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for i, name in enumerate(model_names):
                writer.writerow(
                    [name] + [f"{mat[i, j]:.4f}" for j in range(num_models)]
                )
    ...
    try:
        ...
        fig, axes = plt.subplots(
            1, 2, figsize=(max(14, num_models * 2), max(6, num_models * 0.8))
        )

        for ax, mat, title_tag in zip(
            axes,
            [cka_matrix, cka_matrix_gap],
    ...
    for tag, mat in [
        ("stage5_out (pooled)", cka_matrix),
        ("refine_out (GAP)", cka_matrix_gap),
    ]:
        print("-" * 70)
        ...
        for i, name in enumerate(model_names):
            row = f"{name:<{col_w}}" + "".join(
                [f"{mat[i,j]:{col_w}.4f}" for j in range(num_models)]
            )
            print(row)
...
def get_args():
    p = argparse.ArgumentParser(
        "CKA Analysis for Model Similarity (Multi-Model Support)"
    )
