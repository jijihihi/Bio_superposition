...
def main():
    ...
    sample_x = next(iter(loader))[0][:1].to(device).contiguous(memory_format=torch.channels_last)
    ...
    for batch in tqdm(loader, desc="GAP", leave=True):
        ...
        if x.numel() < 1:
            continue
        x_dev = x.to(device, non_blocking=True).contiguous(memory_format=torch.channels_last)
        with torch.amp.autocast(**autocast_kwargs):
    ...
    all_same = collect_spatial_averaged(
        lambda x: patch_shuffle_all_same_perm(x, ps), desc="all_same_perm", n_repeats=n_rep)
    ...
    for metric in ["l2sq", "gap"]:
        ...
        logger.info(f"  {metric_label}(all_same_perm) mean: {tk_all_same[alive_mask].mean():.6f}")
