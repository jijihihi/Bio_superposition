    ...
    def apply(self, img_np: np.ndarray) -> np.ndarray:
        ...
        if is_16bit:
            ...
            filtered_8bit = cv2.bilateralFilter(
                img_8bit, d=self.d, sigmaColor=self.sigma_color, sigmaSpace=self.sigma_space
            )
            # Scale back to 16-bit
            filtered = (filtered_8bit.astype(np.float32) / 255.0 * 65535.0).astype(np.uint16)
        else:
            filtered = cv2.bilateralFilter(
                img_np, d=self.d, sigmaColor=self.sigma_color, sigmaSpace=self.sigma_space
            )
        return filtered
...
class FastNLMeansDenoising:
    ...
    """
    def __init__(self, h: float = 0.1, patch_size: int = 5, patch_distance: int = 6, p: float = 1.0):
        """
