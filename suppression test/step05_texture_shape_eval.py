...
# Import transforms
from step02_transform import (
    BilateralFilter, GaussianBlur, BoxBlur, MedianFilter,
    FastNLMeansDenoising, PatchShuffle,
)

# Import hyperparameters
from step03_hyperparameters import (
    METRIC_PARAMS, BILATERAL_PARAMS, GAUSSIAN_PARAMS,
    NLMEANS_PARAMS, BOX_PARAMS, MEDIAN_PARAMS, PATCH_SHUFFLE_PARAMS,
    EVAL_SETTINGS, get_all_params,
)

# Import metrics
from step04_evaluate_transform import (
    compute_LV, compute_HFE, compute_ESSIM, compute_GC,
)
...
def build_transform_configs():
    ...
    for p in BILATERAL_PARAMS["diagonal_sweep"]:
        configs.append({
            ...
            "transform": BilateralFilter(
                d=p["k"], sigma_color=p["sigma_color"],
                sigma_space=BILATERAL_PARAMS["sigma_space"], p=1.0
            ),
        })
    ...
    configs.append({
        ...
        "transform": PatchShuffle(grid_size=PATCH_SHUFFLE_PARAMS["grid_size"], p=1.0),
    })
...
def sample_per_class(samples: List[Dict], n_per_class: int, seed: int = 42) -> List[Dict]:
...
def evaluate_image(img: np.ndarray, configs: List[Dict]) -> List[Dict]:
    ...
    for cfg in configs:
        try:
            ...
            if img.ndim == 3:
                filtered = np.stack([transform.apply(img[:, :, c]) for c in range(img.shape[2])], axis=2)
            ...
            results.append({
                ...
                "LV": lv, "HFE": hfe, "ESSIM": essim, "GC": gc,
                "Texture": (lv + hfe) / 2,
                "Shape": (essim + gc) / 2,
            })
        except Exception as e:
...
def run_evaluation(args):
    ...
    logger.info(f"Processed {processed_count} images, skipped {skipped_tar} tars, {skipped_img} images")
    ...
    summary = df.groupby(["transform", "type", "category"]).agg({
        "LV": ["mean", "std"],
        "HFE": ["mean", "std"],
        "ESSIM": ["mean", "std"],
        "GC": ["mean", "std"],
...
def print_results(summary, n_images, n_per_class, output_dir):
    ...
    print(f"Metric params: w={METRIC_PARAMS['window_size']}, r={METRIC_PARAMS['freq_radius']}, k={METRIC_PARAMS['sobel_ksize']}")
    print("=" * 80)
    ...
    for cat_key, cat_name in categories:
        ...
        cols = ["transform", "LV_mean", "HFE_mean", "Texture_mean", "ESSIM_mean", "GC_mean", "Shape_mean"]
...
def get_args():
    ...
    p.add_argument("--val_dir", type=str, required=True,
                   help="Directory containing val_split.csv")
