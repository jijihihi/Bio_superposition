...
from suppression_test.step08_channel_attribution import (
    ...
    build_lr_seam_mask, build_ud_seam_mask,
...
def main():
    ...
    lr_mask = build_lr_seam_mask(Hf, Wf, margin, device)  # (1,1,H,W), masks vertical center
    ud_mask = build_ud_seam_mask(Hf, Wf, margin, device)  # (1,1,H,W), masks horizontal center
    ...
    def collect_spatial_averaged(perturb_fn, desc="", n_repeats=1, mask=None):
        ...
        for rep in range(n_repeats):
            data = collect_spatial(perturb_fn,
                                   desc=f"{desc} rep{rep+1}/{n_repeats}",
                                   mask=mask)
    ...
    G_lr__lr_mask = collect_spatial(
        lambda x: lr_swap_channels(x, [1]),
        desc="G_lr (lr_mask)", mask=lr_mask)
    Rlr_Gud__lr_mask = collect_spatial(
        lambda x: ud_swap_channels(lr_swap_channels(x, [0]), [1]),
        desc="Rlr_Gud (lr_mask)", mask=lr_mask)

    # ud pair: both have G ud seam (common in G_ud), R has ud seam in all_broken → ud_mask
    G_ud__ud_mask = collect_spatial(
        lambda x: ud_swap_channels(x, [1]),
        desc="G_ud (ud_mask)", mask=ud_mask)
    Rud_Glr__ud_mask = collect_spatial(
        lambda x: lr_swap_channels(ud_swap_channels(x, [0]), [1]),
        desc="Rud_Glr (ud_mask)", mask=ud_mask)
    ...
    B_lr__combined = collect_spatial(
        ...
        desc="B_lr (combined_mask)", mask=combined_mask)
    Rlr_Gud__combined = collect_spatial(
        lambda x: ud_swap_channels(lr_swap_channels(x, [0]), [1]),
        desc="Rlr_Gud (combined_mask)", mask=combined_mask)

    # ud pair: B_ud has no seam, Rud_Glr has R ud + G lr → mask both
    B_ud__combined = collect_spatial(
        lambda x: ud_swap_channels(x, [2]),
        desc="B_ud (combined_mask)", mask=combined_mask)
    Rud_Glr__combined = collect_spatial(
        lambda x: lr_swap_channels(ud_swap_channels(x, [0]), [1]),
        desc="Rud_Glr (combined_mask)", mask=combined_mask)

    # ──────────────────────────────────────────────────────────────
    # 3. Local+Texture (no mask)
    # ──────────────────────────────────────────────────────────────
    logger.info("\n  ── Single-channel shifts (no mask, for local+tex) ──")
    R_only_lr_nm = collect_spatial(lambda x: lr_swap_channels(x, [0]),
                                   desc="R_only_lr (no mask)", mask=None)
    R_only_ud_nm = collect_spatial(lambda x: ud_swap_channels(x, [0]),
                                   desc="R_only_ud (no mask)", mask=None)
    G_only_lr_nm = collect_spatial(lambda x: lr_swap_channels(x, [1]),
                                   desc="G_only_lr (no mask)", mask=None)
    G_only_ud_nm = collect_spatial(lambda x: ud_swap_channels(x, [1]),
                                   desc="G_only_ud (no mask)", mask=None)
    B_only_lr_nm = collect_spatial(lambda x: lr_swap_channels(x, [2]),
                                   desc="B_only_lr (no mask)", mask=None)
    B_only_ud_nm = collect_spatial(lambda x: ud_swap_channels(x, [2]),
                                   desc="B_only_ud (no mask)", mask=None)
    ...
    baseline = collect_spatial_averaged(
        ...
        desc="baseline", n_repeats=n_rep, mask=None)
    ...
    for metric in ["l2sq", "gap"]:
        ...
        logger.info(f"  NOTE: interactions use seam-masked GAP, local+tex use full GAP.")
        ...
        for n_i in range(d_sae):
            ...
            gb_lr_i = R_lr__ud_mask[metric][idx, n_i] - Rlr_Gud__ud_mask[metric][idx, n_i]
            gb_ud_i = R_ud__lr_mask[metric][idx, n_i] - Rud_Glr__lr_mask[metric][idx, n_i]
            gb_i = (gb_lr_i + gb_ud_i) / 2

            rb_lr_i = G_lr__lr_mask[metric][idx, n_i] - Rlr_Gud__lr_mask[metric][idx, n_i]
            rb_ud_i = G_ud__ud_mask[metric][idx, n_i] - Rud_Glr__ud_mask[metric][idx, n_i]
            rb_i = (rb_lr_i + rb_ud_i) / 2

            rg_lr_i = B_lr__combined[metric][idx, n_i] - Rlr_Gud__combined[metric][idx, n_i]
            rg_ud_i = B_ud__combined[metric][idx, n_i] - Rud_Glr__combined[metric][idx, n_i]
        ...
        for blo, bhi, label in [
            (0.8, 1.2, "0.8-1.2"), (0.5, 1.5, "0.5-1.5"), (0.9, 1.1, "0.9-1.1"),
        ]:
