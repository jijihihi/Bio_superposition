...
def get_args():
    ...
    p.add_argument("--sae_cache", type=str, required=True,
    ...
    p.add_argument("--dot_size", type=float, default=7.0,
                   help="Scatter dot size. Default: 7")
    p.add_argument("--alpha", type=float, default=0.5,
                   help="Dot transparency (0-1). Default: 0.5")
    p.add_argument("--cmap", type=str, default="magma_r",
                   help="Colormap (legacy). Default: magma_r")
    p.add_argument("--cmap_mean", type=str, default="magma_r",
                   help="Colormap for apoptosis rate. Default: magma_r")
    p.add_argument("--cmap_std", type=str, default="inferno",
                   help="Ignored but kept for argument compatibility.")
    ...
    p.add_argument("--vmax_pctl", type=float, default=90.0,
...
def plot_mutation_umap(embedding, apoptosis, mutation, args, out_dir):
    ...
    logger.info(f"    [{mutation}] Drawing UMAP scatter: vmin={vmin:.4f}, vmax={vmax:.4f}, gamma={args.gamma}")
    ...
    if args.cmap_start > 0.0 or args.cmap_end < 1.0:
        actual_cmap = truncate_cmap(args.cmap_mean, args.cmap_start, args.cmap_end)
    else:
        actual_cmap = plt.get_cmap(args.cmap_mean)

    sc = ax.scatter(embedding[:, 0], embedding[:, 1], c=apoptosis, cmap=actual_cmap,
                    s=args.dot_size, alpha=args.alpha, norm=norm,
                    edgecolors='none', rasterized=True)
                    
    ax.set_title(f"{mutation} UMAP — Apoptosis Rate", fontsize=14, fontweight='bold', color=txt_color)
    ...
    cbar = fig.colorbar(sc, ax=ax, shrink=0.8, pad=0.03, label=f"Apoptosis Rate{gamma_label}")
    ...
    # Save files
    param_suffix = f"nn{args.umap_n_neighbors}_md{args.umap_min_dist}_{args.umap_metric}"
    base_path = os.path.join(out_dir, f"umap_apoptosis_{mutation}_{param_suffix}")
    
    fig.savefig(f"{base_path}.png", dpi=args.dpi, bbox_inches="tight", facecolor=args.bg_color)
    fig.savefig(f"{base_path}.svg", format='svg', bbox_inches="tight", facecolor=args.bg_color)
    fig.savefig(f"{base_path}.pdf", format='pdf', bbox_inches="tight", facecolor=args.bg_color)
...
def main():
    ...
    if not args.output_dir:
        ...
        args.output_dir = os.path.join(os.path.dirname(args.sae_cache), "umap_mutations")
    ...
    apoptosis = load_and_match_apoptosis(args.apoptosis_csv, uids, rate_col=args.rate_col)
    ...
    if len(valid_apop_all) > 0:
        ...
        logger.info(f"Global apoptosis color scale: vmin={args.global_vmin:.4f}, vmax={args.global_vmax:.4f}")
    else:
    ...
    if len(valid_apop) > 0:
        logger.info(f"  Apoptosis rates: min={valid_apop.min():.4f}, max={valid_apop.max():.4f}, "
    ...
    for mutation in args.classes:
        ...
        if len(X_mut) < 10:
            logger.warning(f"Not enough valid samples for {mutation} (N={len(X_mut)}). Skipping.")
            continue
            
        logger.info(f"  Apoptosis rates for {mutation}: min={apop_mut.min():.4f}, max={apop_mut.max():.4f}, "
