...
def get_args():
    ...
    p.add_argument(
        "--gap_l2_norm",
        action="store_true",  # CNN 아니고 SAE 인 경우에도 적용된다. SAE 인 경우에는 전체 GAP L2 정규화를 할 이유가 없다. CNN인 경우에 하는 것은 세포양 보정 때문이고 SAE에 넣어줄떄 이미 L2 정규화를 했기 때문에.
    ...
    p.add_argument(
        "--xgb_gamma", type=float, default=0.3
    )  # 감마가 클수록 보수적인 모델이 됨
    p.add_argument(
        "--xgb_min_child_weight", type=int, default=3
    )  # 자식 노드에 필요한 최소 가중치 합
    ...
    for fm in filter_mode:
        if fm == "none":
            ...
            if de_top_k > 0 and de_mask.sum() > de_top_k:
                ...
                logger.info(
                    f"    [filter] DE top_k={de_top_k}: kept {int(de_mask.sum())}"
                )
...
def plot_permutation_null(real_r2, perm_r2s, group, pval, output_path, dpi=200):
    ...
    ax.set_title(
        f"{group} — Permutation Test (p={_fmt_p(pval)})", fontsize=13, fontweight="bold"
    )
...
def evaluate_r2(
    X, y, seed, cv_folds, n_permutations=0, n_repeats=1, pca_dim=0, debug=True
):
    """
    ...
    def _make_kf():
        ...
        if n_repeats > 1:
            return RepeatedKFold(
                n_splits=cv_folds, n_repeats=n_repeats, random_state=seed
            )
        ...
        for ri in range(n_repeats):
            ...
            for fold_i in range(cv_folds):
                ...
                if log_folds:
                    ...
                    logger.info(
                        f"      Repeat {ri} Fold {fold_i}: n_train={len(train_idx)}, "
    ...
    # Real run
    logger.info(
        f"    X shape: {X_v.shape}" + (f" → PCA {pca_dim}" if pca_dim > 0 else "")
    )
    ...
    logger.info(
        f"    R² repeats_mean={r2_mean:.4f} ± {r2_std:.4f}, "
    ...
    if n_permutations > 0:
        ...
        logger.info(
            f"    Permutation: {n_permutations} perms × {cv_folds} folds "
        ...
        perm_fold_r2s = np.array(
            [r2 for fold_list, _ in results_list for r2 in fold_list]
        )
...
# ==============================================================================
def evaluate_r2_xgboost(
    X, y, seed, cv_folds, args, n_permutations=0, n_repeats=1, pca_dim=0, debug=True
):
    """
    ...
    def _cv_pass(X_in, y_in, log_folds=False):
        ...
        for ri in range(n_repeats):
            ...
            for fold_i in range(cv_folds):
                ...
                val_idx = np.random.RandomState(seed + fold_i).permutation(n_train)[
                    :n_val
                ]
                ...
                if log_folds:
                    best_iter = (
                        ...
                        else args.xgb_n_estimators
                    )
                    logger.info(
                        f"      Repeat {ri} Fold {fold_i}: n_train={len(tr_idx)}, "
        ...
        def _single_perm_xgb(perm_seed):
            ...
            for ri in range(n_repeats):
                ...
                for fold_i in range(cv_folds):
                    ...
                    n_train = len(X_train)
                    n_val = max(1, int(n_train * 0.15))
                    val_idx = np.random.RandomState(perm_seed + fold_i).permutation(
                        n_train
                    )[:n_val]
                    tr_idx = np.setdiff1d(np.arange(n_train), val_idx)
...
def main():
    ...
    if "X_gap" in data:
        ...
        raise ValueError(
            f"Unknown cache format. Keys: {cache_keys}. "
    ...
    if has_filter:
        logger.info(
            f"Filter: {args.filter_mode}, min_cv={args.min_cv}, "
    ...
    for mut in mutations:
        ...
        if has_filter:
            ...
            if n_features < 2:
                logger.warning(
                    f"    Feature selection returned {n_features} features, "
        ...
        results.append(res)
        logger.info(
            f"  {mut+' only':12s}:  R² = {res['r2_mean']:.4f} ± {res['r2_std']:.4f} "
    ...
    if not args.quiet:
        plot_r2_summary(
            results,
            os.path.join(out_dir, f"r2_summary_{which_layer}.png"),
            dpi=args.dpi,
        )
        ...
        for res in results:
            if len(res.get("y_true", [])) > 10:
                ...
                plot_pred_vs_actual(
                    ...
                    res["perm_pval"],
                    os.path.join(
                        out_dir, f"pred_vs_actual_{safe_name}_{which_layer}.png"
                    ),
                    dpi=args.dpi,
                )
                if len(res.get("perm_r2s", [])) > 0:
                    plot_permutation_null(
                        ...
                        res["perm_pval"],
                        os.path.join(
                            out_dir, f"perm_null_{safe_name}_{which_layer}.png"
                        ),
                        dpi=args.dpi,
                    )
    ...
    for res in results:
        logger.info(
            f"  {res['group']:15s}: R² = {res['r2_mean']:.4f} ± {res['r2_std']:.4f}  "
            f"p = {_fmt_p(res['perm_pval'])}"
        )
    ...
    for res in results:
        jr = {
            ...
            "perm_pval": (
                float(res["perm_pval"]) if res.get("perm_pval") is not None else None
            ),
        ...
        json.dump(
            {
                ...
                # ── Ridge ──
                "ridge_alpha_range": [
                    float(_RIDGE_ALPHAS[0]),
                    float(_RIDGE_ALPHAS[-1]),
                ],
