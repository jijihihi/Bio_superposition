...
def plot_cv_comparison(results_by_source, mutation, k, output_path, dpi=200):
    ...
    ax.set_title(
        f"{mutation} — KNN Distance Uniformity (k={k})", fontsize=13, fontweight="bold"
    )
...
def plot_mean_dist_comparison(results_by_source, mutation, k, output_path, dpi=200):
    ...
    ax.set_title(
        f"{mutation} — Mean KNN Distance (k={k})", fontsize=13, fontweight="bold"
    )
    ax.grid(True, alpha=0.2, axis="y")
...
def plot_k_sweep(sweep_results, mutation, output_path, dpi=200):
    ...
    for source_label, kv_list in sweep_results.items():
        ...
        ax.plot(
            ...
            linewidth=1.5,
            markersize=5,
            label=f"{source_label} (median CV)",
            alpha=0.6,
        )
...
def plot_k_sweep_std(sweep_results, mutation, output_path, dpi=200):
    ...
    for source_label, kv_list in sweep_results.items():
        ...
        ax.plot(
            ...
            linewidth=1.5,
            markersize=5,
            label=f"{source_label} (median std)",
            alpha=0.6,
        )

    ax.set_xlabel("k (number of neighbors)", fontsize=11)
    ax.set_ylabel("KNN Distance Std", fontsize=11)
    ax.set_title(f"{mutation} — KNN Distance Std vs k", fontsize=13, fontweight="bold")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.2)
    ax.xaxis.set_major_locator(mticker.MaxNLocator(integer=True))
...
def main():
    ...
    def _load_and_preprocess(cache_path):
        ...
        if "X_gap" in data:
            ...
            X, _, lines, uids, _, _ = load_features_cache(
                cache_path, args.dead_threshold
            )
            label = "SAE"
        else:
    ...
    k_sweep_data = (
        {}
    )  # mutation → {source → [(k, mean_cv, med_cv, mean_std, med_std), ...]}
    ...
    for source_label, (X_raw, superclasses, uids) in sources.items():
        ...
        if has_de and de_mode == "union":
            ...
            logger.info(
                f"    DE(Ctrl vs AllMut): Control-high={int(ctrl_high_mask.sum())}"
            )
        ...
        for mut in mutations:
            ...
            for k in args.k_neighbors:
                ...
                logger.info(
                    f"      Distance: mean={agg_mean_dist:.6f}, "
                    f"median={agg_median_dist:.6f}"
                )
                logger.info(
                    f"      Dist Std: mean={agg_mean_std:.6f}, "
                    f"median={agg_median_std:.6f}"
                )
                logger.info(
                    f"      Dist CV:  mean={agg_mean_cv:.4f}, "
