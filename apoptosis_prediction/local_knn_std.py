...
def main():
    ...
    for source_label, (X_raw, superclasses, uids, apoptosis) in sources.items():
        ...
        # ══════════════════════════════════════════════════════════════
        # Global filters (gini / cv) — identical to dpt_kendall run_analysis()
        # ══════════════════════════════════════════════════════════════
        ...
        if de_eval_split > 0 and has_de:
            ...
            eval_mask = np.zeros(n_total, dtype=bool)  # True = eval set
            # Stratified split by superclass
        ...
        # DE union mode: apply global DE union before norm/PCA
        # Uses only DE set for feature selection, applies mask to ALL data
        if has_de and de_mode == "union":
            de_masks = []
            # Per-mutation DE — using DE set only
