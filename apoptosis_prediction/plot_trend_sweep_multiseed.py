...
def main():
    ...
    for mut in mutations:
        ...
        for seed in seeds:
            ...
            for d in dims:
                json_path = os.path.join(
                    base_dir, f"seed_{seed}", f"d{d}", "local_linearity_results.json"
                )
        ...
        df_std_median.to_csv(
            os.path.join(tables_dir, f"table_knn_std_median_{mut}.csv")
        )
        ...
        has_morans = any(sae_morans_data[d][k] for d in dims for k in ks) or any(
            cnn_morans_data[k] for k in ks
        )

        if has_morans:
            df_moran_mean = build_dataframe(sae_morans_data, cnn_morans_data, np.mean)
            df_moran_median = build_dataframe(
                sae_morans_data, cnn_morans_data, np.median
            )

            # CSV 파일 저장
            df_moran_mean.to_csv(
                os.path.join(tables_dir, f"table_morans_I_mean_{mut}.csv")
            )
            df_moran_median.to_csv(
                os.path.join(tables_dir, f"table_morans_I_median_{mut}.csv")
            )
