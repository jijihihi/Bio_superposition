...
# CNN seed 87 고정, SAE seed만 변경. DPT 하게.
SCRIPT="python -m sae_project.step09_train_gated_sae"
CNN_SEED=87
SAE_SEEDS=(777 856)
BASE="/home/ubuntu/model-east3/outputs"
...
for SAE_SEED in "${SAE_SEEDS[@]}"; do
    echo "===== CNN=${CNN_SEED}, SAE seed=${SAE_SEED} ====="
    
    $SCRIPT \
        --save_dir "$MODEL_DIR" \
        --model_state_path "${MODEL_DIR}/best_model.pt" \
        --sae_save_dir "${MODEL_DIR}/SAE_seed${SAE_SEED}_no_L2norm_loss" \
        --shard_root /home/ubuntu/model-east3/wds_shards_tar \
        --which_layer stage5_out \
        --use_bf16 \
        --batch_size 64 \
        ...
        --sparsity_warmup_steps 100 \
        --seed "$SAE_SEED" \
        2>&1 | tee "${MODEL_DIR}/SAE_seed${SAE_SEED}_no_L2norm_loss/sae_train_log.txt"
