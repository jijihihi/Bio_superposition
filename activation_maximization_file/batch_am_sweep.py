...
def visualize_am_with_heatmap(
    ...
    fig.suptitle(f"SAE Neuron {neuron_idx:04d} — act={activation:.4f}",
...
def get_args():
    ...
    p.add_argument("--decay_powers", type=str, default="1.0",
                   help="Comma-separated decay_power values to sweep")
    p.add_argument("--jitter_pxs", type=str, default="8",
...
def main():
    ...
    if args.concept_dir:
        ...
        logger.info(f"Auto-discovered {len(concepts_with_dirs)} concepts from {args.concept_dir}")
    ...
    if len(decay_powers) != len(jitter_pxs):
        logger.warning(f"decay_powers ({len(decay_powers)}) and jitter_pxs ({len(jitter_pxs)}) "
    ...
    logger.info(f"Total runs: {len(concepts_with_dirs)} concepts × {len(hp_combos)} HP combos × {len(seeds)} seeds = {total_runs}")
        ...
        for decay_power, jitter_px in hp_combos:
            ...
            for seed in seeds:
                ...
                if os.path.exists(npz_path):
                    logger.info(f"  [{run_count}/{total_runs}] SKIP (exists): {npz_path}")
                    continue
                ...
                visualize_am_with_heatmap(
                    img, neuron_idx, act, heatmap, png_path, dpi=args.dpi,
                )
    ...
    if args.drive_backup_dir:
        ...
        if src_files == dst_files:
            ...
            logger.warning(
                f"  ⚠ File count mismatch! src={src_files}, dst={dst_files}"
            )
