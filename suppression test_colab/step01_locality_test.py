...
def plot_perturbation_curve(
    ...
    if 1 in grid_sizes:
        ...
        plt.axhline(y=baseline_acc, color='r', linestyle='--', alpha=0.5, label=f'Baseline: {baseline_acc:.1f}%')
...
def visualize_shuffled_examples(
    ...
    for idx in indices[:num_examples]:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"example_{idx}_{class_name}.png"), dpi=100, bbox_inches="tight")
        plt.close()
...
def run_perturbation_test(args):
    ...
    train_ds = InMemorySixteenBitDataset(train_bank, train_ib, args.img_size, grid_shuffle=None, augment=False)
    test_ds = InMemorySixteenBitDataset(test_bank, test_ib, args.img_size, grid_shuffle=None, augment=False)
    
    train_loader = DataLoader(
        train_ds, batch_size=args.batch_size, shuffle=False,
        num_workers=0, pin_memory=True, collate_fn=collate_skip_none
    )
    ...
    train_features, train_labels = extract_features(encoder, train_loader, device, args.use_bf16)
    logger.info(f"Train features shape: {train_features.shape}")
    
    # ==== Train linear probe ====
    logger.info("Training linear probe...")
    linear_probe = train_linear_probe(train_features, train_labels, args.num_classes, args)
    ...
    test_features, test_labels = extract_features(encoder, test_loader, device, args.use_bf16)
    baseline_acc, baseline_preds, _ = evaluate_probe(linear_probe, test_features, test_labels, device)
    
    logger.info(f"Baseline Test Accuracy: {baseline_acc * 100:.2f}%")
    
    # Classification report
    class_names = ["Control", "SNCA", "GBA", "LRRK2"]
    report = classification_report(test_labels, baseline_preds, target_names=class_names)
    logger.info(f"\nClassification Report:\n{report}")
        ...
        if grid_size == 1:
            ...
            for trial in range(args.num_shuffle_trials):
                ...
                test_ds_shuffled = InMemorySixteenBitDataset(
                    test_bank, test_ib, args.img_size,
                    grid_shuffle=grid_size, augment=False
                )
                test_loader_shuffled = DataLoader(
                    test_ds_shuffled, batch_size=args.batch_size, shuffle=False,
                    num_workers=0, pin_memory=True, collate_fn=collate_skip_none
                )
                ...
                # Evaluate
                acc, _, _ = evaluate_probe(linear_probe, shuffled_features, shuffled_labels, device)
...
def get_args():
    ...
    p.add_argument("--output_dir", type=str, default="/content/drive/MyDrive/Final_paper/Model_MoCoXBM_PlateLP_LRRK2_L2 norm_hidden2048_resume_bias=True_clean_image/perturbation_test",
                   help="Output directory for results")
    
    # Grid shuffle settings
    p.add_argument("--grid_sizes", type=str, default="2,3,4,5,6,8,12,16",
    ...
    p.add_argument("--batch_size", type=int, default=128, help="Batch size for feature extraction")
    p.add_argument("--seed", type=int, default=42, help="Random seed")
    
    # Model architecture (must match training)
    p.add_argument("--blocks", type=str, default="2,2,2,3", help="Encoder blocks configuration")
    p.add_argument("--dilations", type=str, default="1,1,1,1", help="Encoder dilations")
    p.add_argument("--refine_blocks", type=int, default=1, help="Number of refine blocks")
    ...
    p.add_argument("--lp_epochs", type=int, default=10, help="Linear probe training epochs")
    p.add_argument("--lp_lr", type=float, default=0.1, help="Linear probe learning rate")
    p.add_argument("--lp_wd", type=float, default=0.0, help="Linear probe weight decay")
    p.add_argument("--lp_momentum", type=float, default=0.9, help="Linear probe momentum (SGD)")
    p.add_argument("--lp_batch_size", type=int, default=4096, help="Linear probe batch size")
