...
def get_args():
    ...
    parser.add_argument("--input_dir", type=str, default=r"C:\Users\admin\Desktop\cropped_image", help="Input directory")
    parser.add_argument("--debug_dir", type=str, default=r"C:\Users\admin\Desktop\cropped_image\debug", help="Output directory for debug images")
    parser.add_argument("--sample_count", type=int, default=50, help="Number of samples")
...
def process_debug_image(img, filename, save_dir):
    ...
    for i in range(c):
        ...
        # Otsu 수행
        otsu_val, _ = cv2.threshold(channel_8bit, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    # 저장
    save_path = os.path.join(save_dir, f"Debug_{bg_frac*100:.0f}pct_{filename[:-4]}.png")
