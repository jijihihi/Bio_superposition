...
def main():
    base_dir = "/content/drive/MyDrive/Final_paper/lambda_labs_moco_only/knn_eval_results"
    ...
    for sdir in seed_dirs:
        ...
        for edir in eval_dirs:
            ...
            if "SAE" in dirname:
                match = re.search(r"(SAE_d\d+_lam\d+(?:_norm_\w+|))_(uniform|inv_sq)", dirname)
                if match:
    ...
    for config in configs:
        for w in weights:
            if w not in results_acc[config]: continue
            for k in ks:
                if k not in results_acc[config][w]: continue
    ...
    for target_k in ks:
        ...
        ax.set_title(f"KNN Accuracy Heatmap (K={target_k})", fontsize=14, fontweight='bold', pad=15)
