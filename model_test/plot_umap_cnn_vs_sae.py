...
def load_cache(path, is_sae=False, dead_threshold=1e-5, gap_l2_norm=False):
    ...
    if is_sae:
        ...
        if "usage_ema" in data:
            ...
            logger.info(f"  Shape: {X.shape} (alive neurons: {alive_mask.sum()}/{len(usage_ema)}, thresh={dead_threshold})")
        else:
...
def main():
    ...
    parser.add_argument("--gap_l2_norm", action="store_true", help="Apply L2 norm to BOTH CNN and SAE")
    parser.add_argument("--cnn_only_l2_norm", action="store_true", help="Apply L2 norm ONLY to CNN (Recommended)")
    parser.add_argument("--n_neighbors", type=int, default=15)
    parser.add_argument("--min_dist", type=float, default=0.1)
    parser.add_argument("--subsample", type=int, default=5000, help="Max cells per class to plot")
    parser.add_argument("--classes", type=int, nargs="+", default=[], help="Specific class numbers to plot (empty = all)")
    ...
    X_sae, y_sae = load_cache(args.sae_cache, is_sae=True, dead_threshold=args.dead_threshold, gap_l2_norm=sae_l2)

    # Validate labels match
    assert np.array_equal(y_cnn, y_sae), "Labels do not match between CNN and SAE caches!"
    ...
    reducer_cnn = umap.UMAP(n_neighbors=args.n_neighbors, min_dist=args.min_dist, random_state=42)
    Z_cnn = reducer_cnn.fit_transform(X_cnn_sub)

    logger.info("Running UMAP for SAE...")
    reducer_sae = umap.UMAP(n_neighbors=args.n_neighbors, min_dist=args.min_dist, random_state=42)
    ...
    for idx, c in enumerate(classes):
        ...
        # Plot CNN
        axes[0].scatter(Z_cnn[mask, 0], Z_cnn[mask, 1], c=[color], label=f"Class {c}", alpha=0.6, s=5)
        
        # Plot SAE
        axes[1].scatter(Z_sae[mask, 0], Z_sae[mask, 1], c=[color], label=f"Class {c}", alpha=0.6, s=5)
