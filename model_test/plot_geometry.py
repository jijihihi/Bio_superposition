...
def scan_results(base_dir):
    ...
    for source in ["CNN", "SAE"]:
        ...
        logger.info(
            f"  {source}: k_vals={k_vals}, " f"seeds_per_k={n_seeds}, δ_seeds={n_delta}"
        )
...
def plot_ricci_vs_k(agg, out_dir, dpi=200):
    ...
    for ax, (field, title, unit) in zip(axes, metrics):
        for source in ["CNN", "SAE"]:
            ...
            ax.fill_between(
                k_vals, means - stds, means + stds, color=colors[source], alpha=0.15
            )

        ax.axhline(
            0 if "frac" not in field else 50,
            color="gray",
            linestyle=":",
            linewidth=1,
            alpha=0.5,
    ...
    fig.suptitle(
        "Ollivier-Ricci Curvature Statistics: CNN vs SAE",
        fontsize=15,
        fontweight="bold",
        y=1.02,
    )
...
def plot_ricci_per_class_vs_k(per_class_data, out_dir, dpi=200):
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(4.5 * ncols, 4.5 * nrows), squeeze=False
    )
    ...
    for idx, cls in enumerate(classes):
        ...
        for source in ["CNN", "SAE"]:
            ...
            for k in k_vals:
                ...
                if not entries:
                    continue
                vals = np.array([e["mean"] for e in entries if not np.isnan(e["mean"])])
                if len(vals) == 0:
                    continue
                valid_ks.append(k)
        ...
        ax.set_title(
            cls, fontsize=12, fontweight="bold", color=class_colors.get(cls, "black")
        )
    ...
    fig.suptitle(
        "Intra-class Ollivier-Ricci Curvature vs K",
        fontsize=14,
        fontweight="bold",
        y=1.01,
    )
    ...
    for ax, source in zip(axes, ["CNN", "SAE"]):
        ...
        ax.set_title(
            f"{source} — Intra-class Curvature", fontsize=12, fontweight="bold"
        )
        ax.legend(fontsize=9, framealpha=0.85)
        ax.grid(True, alpha=0.15)
        sns.despine(ax=ax)
    ...
    fig.suptitle(
        "Per-class Intra-class Curvature: CNN vs SAE",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
...
def plot_delta_comparison(delta_data, out_dir, dpi=200):
    ...
    fig.suptitle(
        "Gromov δ-Hyperbolicity: CNN vs SAE", fontsize=14, fontweight="bold", y=1.02
    )
    fig.tight_layout()
...
def plot_combined_summary(agg, delta_data, out_dir, dpi=200):
    ...
    ax.set_title("(A) Mean Ollivier-Ricci Curvature", fontsize=12, fontweight="bold")
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.15)
    ...
    ax.axhline(
        50, color="gray", linestyle=":", linewidth=1, alpha=0.5, label="50% (balanced)"
    )
    ax.set_xlabel("K", fontsize=11)
    ax.set_ylabel("% edges with κ > 0", fontsize=11)
    ax.set_title(
        "(B) Fraction of Positively Curved Edges", fontsize=12, fontweight="bold"
    )
    ...
    ax.set_title("(D) Normalized δ-Hyperbolicity", fontsize=12, fontweight="bold")
    ax.grid(True, alpha=0.15, axis="y")
    ...
    fig.suptitle(
        "Intrinsic Geometry: CNN vs SAE Representations",
        fontsize=15,
        fontweight="bold",
        y=1.01,
    )
...
def compute_global_correlations_and_plot(base_dir, out_dir, dpi=200):
    ...
    for source, pattern in [("CNN", "cnn_seed_*"), ("SAE", "sae_seed_*")]:
        ...
        for seed_dir in seed_dirs:
            ...
            npz_cands = glob.glob(
                os.path.join(seed_dir, f"pairwise_{source}_combined.npz")
            )
            if not npz_cands:
            ...
            if npz_cands:
                try:
                    ...
                    if N_total > 0:
                        ...
                        idx_plot = rng.choice(
                            N_total,
                            min(N_total, n_sample_plot // len(seed_dirs)),
                            replace=False,
                        )
    ...
    for col_i, source in enumerate(["CNN", "SAE"]):
        ...
        ax.scatter(
            x_cls, y_cls, c=color, alpha=0.12, s=1.0, linewidths=0, rasterized=True
        )

        has_gam = False
        adj_r2 = np.nan
        try:
            x_pred, y_line, y_lo, y_hi, adj_r2 = fit_gam(
                x_cls, y_cls, n_splines=12, lam=0.6, degree=3
            )
            ax.plot(
                x_pred,
                y_line,
        ...
        ax.set_title(
            f"{source}  │  All Classes (Inter + Intra)", fontsize=12, fontweight="bold"
        )
        if has_gam:
...
def main():
    ...
    for source in ["CNN", "SAE"]:
        ...
        seen = set()
        unique = [
            e for e in entries if e["seed"] not in seen and not seen.add(e["seed"])
        ]
        if unique:
            deltas = np.array([e["delta"] for e in unique])
            d_rels = np.array([e["delta_rel"] for e in unique])
            logger.info(
                f"  {source}: δ = {np.mean(deltas):.4f}±{np.std(deltas):.4f}, "
