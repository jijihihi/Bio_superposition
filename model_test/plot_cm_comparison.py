...
def get_args():
    p = argparse.ArgumentParser(
        description="Confusion Matrix Comparison — CNN vs SAE"
    )
    p.add_argument("--results_dir", type=str, required=True,
                   help="Root directory containing CNN_seed*/SAE_sp*_seed*/ subdirs")
    p.add_argument("--cnn_seed", type=int, required=True,
                   help="CNN encoder seed to display (e.g. 87)")
    p.add_argument("--sae_seed", type=int, required=True,
                   help="SAE seed to display (e.g. 856)")
    ...
    p.add_argument("--alpha", type=float, default=1.0,
                   help="Heatmap alpha. Default: 1.0")
    ...
    p.add_argument("--annot_fontsize", type=float, default=14,
...
def load_single_cm(results_dir, model_type, seed, k):
    ...
    candidates = [d for d in os.listdir(results_dir)
                  if d.startswith(model_type) and d.endswith(f"seed{seed}")]

    if not candidates:
        # Try broader match
        candidates = [d for d in os.listdir(results_dir)
                      if model_type in d and f"seed{seed}" in d]
    ...
    logger.info(f"  Loaded {model_type} seed={seed} k={k}: "
                f"total={cm.sum()}, acc={acc:.1%}")
...
# ==============================================================================
def plot_cm_comparison(cnn_pct, sae_pct,
                       cnn_acc, sae_acc,
                       cnn_seed, sae_seed, k,
                       cmap, alpha, output_path, dpi=200,
                       annot_fontsize=14, figscale=1.0):
        ...
        for i in range(NUM_CLASSES):
            for j in range(NUM_CLASSES):
                ...
                ax.text(j, i, f"{val:.1f}%",
                        ha="center", va="center",
                        fontsize=annot_fontsize, fontweight=weight, color=tc)
    ...
    fig.suptitle(f"KNN Confusion Matrix (k={k})",
...
def main():
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"  CM Comparison — CNN seed={args.cnn_seed}, "
    ...
    plot_cm_comparison(
        cnn_pct, sae_pct,
        cnn_acc, sae_acc,
        args.cnn_seed, args.sae_seed, args.k,
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["model", "seed", "true_class"] +
                        [f"pred_{c}" for c in CLASS_NAMES] +
                        ["accuracy"])
        for model, seed, pct in [("CNN", args.cnn_seed, cnn_pct),
                                  ("SAE", args.sae_seed, sae_pct)]:
            for i, name in enumerate(CLASS_NAMES):
                writer.writerow([model, seed, name] +
