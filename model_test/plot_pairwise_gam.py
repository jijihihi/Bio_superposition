...
def get_args():
    ...
    p.add_argument("--sae_seed_idx", type=int, default=0,
                   help="Same as --cnn_seed_idx but for SAE.")

    # ── Filtering ──
    p.add_argument("--intra_only", action="store_true",
                   help="Use only intra-class pairs.")
    p.add_argument("--classes", nargs="+", default=None,
                   help="Filter to specific class names.")

    # ── Sampling ──
    p.add_argument("--n_sample", type=int, default=80_000,
                   help="Points to show in scatter (stats still use ALL data).")
    p.add_argument("--n_corr_subsample", type=int, default=3_000_000,
                   help="Max pairs for per-seed/per-class correlation (Kendall is slow).")
    p.add_argument("--seed", type=int, default=42)

    # ── GAM ──
    p.add_argument("--n_splines", type=int, default=20,
                   help="Number of splines for GAM.")
    p.add_argument("--gam_lam",   type=float, default=0.6,
                   help="GAM regularisation lambda (pygam).")
    p.add_argument("--poly_degree", type=int, default=3,
                   help="Spline degree (scipy fallback).")
    p.add_argument("--gam_clip_pct", type=float, default=1.0,
                   help="Clip lower+upper X%% of cosine dist before GAM fitting. "
                        "0 = no clip. Default 1.0 removes extreme outliers.")
    p.add_argument("--gam_ci_width", type=float, default=0.95,
                   help="Confidence interval width for GAM band (0 = no CI).")
    p.add_argument("--n_anchor_sub", type=int, default=0,
...
def load_npz(path, args):
    ...
    ci = np.asarray(d.get("class_i", d.get("i_class",
                    np.full(len(cos), "?"))), dtype=str)
    cj = np.asarray(d.get("class_j", ci), dtype=str)
    pt = np.asarray(d.get("pair_type", np.where(ci == cj,
                    "intra", "inter")), dtype=str)
...
def sweep_all_seeds(seed_paths, args):
    ...
    for seed_idx, path in enumerate(seed_paths):
        ...
        rows.append(row_all)
        print(f"      [_ALL_] ρ={stats_all['spearman']:.4f}  "
...
def summarise_rows(rows, label=""):
    ...
    for cls in all_classes + ["_ALL_"]:
        ...
        print(f"  {cls:<20s}  {fmt('spearman'):>14s}  "
...
def print_cnn_sae_comparison(cnn_summ, sae_summ):
    ...
    for cls in all_cls:
        ...
        print(f"  {cls:<20s}  {c_rho:>10.4f}  {s_rho:>10.4f}  "
...
def _resolve_seed_path(paths, spec, model_label=""):
    ...
    if spec >= len(paths):
        print(f"  [{model_label}] WARNING: spec={spec} not found in dir names "
...
def fit_gam(cos, dpt, n_splines=20, lam=0.6, degree=3, seed=42,
    ...
    if _HAS_PYGAM:
        ...
        if len(x_uniq) < degree + 1:
            ...
            try:
                spl   = UnivariateSpline(x_uniq, y_uniq,
                                         k=min(degree, 5), s=s_val)
                yfit  = spl(xf)
                yline = spl(x_pred)
            except Exception:
        ...
        if ci_width > 0:
            ...
            for _ in range(n_boot):
                ...
                ybs = np.bincount(inv_b,
                                  weights=yb[np.argsort(xb)]) / np.bincount(inv_b)
                try:
                    sb = UnivariateSpline(ubs, ybs,
                                         k=min(degree, 5), s=s_val)
...
def compute_rowwise_correlation(npz_path, args):
    ...
    if M != M_check:
        ...
        results[cls] = dict(mean_sp=rho_g, std_sp=np.nan, sem_sp=np.nan,
                            mean_pr=pr_g,  std_pr=np.nan, sem_pr=np.nan,
                            n_anchors=1, method="global_fallback")
...
def sliding_window_corr(cos, dpt, window_size, step, min_pairs):
    starts = np.arange(cos.min(),
                       cos.max() - window_size + step, step)
    mids, pear, spear = [], [], []
    for lo in starts:
        hi = lo + window_size
        mask = (cos >= lo) & (cos < hi)
...
def plot_sliding_window(display_data, args, out_dir):
    ...
    for label, (cos, dpt, used_seed) in display_data.items():
        ...
        mids, pear, spear = sliding_window_corr(
            cos, dpt, args.window_size, args.window_step,
            args.window_min_pairs)
        color = colors.get(label, "#666")
        ax_p.plot(mids, pear,  color=color, linewidth=2, marker="o",
                  markersize=3.5, alpha=0.9, label=f"{label} s{used_seed}")
        ax_s.plot(mids, spear, color=color, linewidth=2, marker="s",
                  markersize=3.5, alpha=0.9, label=f"{label} s{used_seed}")
        np.savez(os.path.join(out_dir,
                              f"sliding_{label}_seed{used_seed}.npz"),
                 midpoints=mids, pearson=pear, spearman=spear)
...
def plot_kde_cosine(seed_paths_by_model, args, out_dir):
    ...
    for model, paths in seed_paths_by_model.items():
        ...
        for path in paths:
            try:
                ...
                all_cos_m.extend(cos[rng.choice(len(cos), n_all,
    ...
    for model, cos_arr in cls_all.items():
        if len(cos_arr) == 0:
            continue
        sns.kdeplot(cos_arr, ax=ax2, fill=True, alpha=0.4,
                    color=colors.get(model, "#999"), linewidth=2.5,
                    label=model, bw_adjust=0.5, clip=(0, None))
        med = float(np.median(cos_arr))
        ax2.axvline(med, color=colors.get(model, "#999"),
                    linewidth=1.5, linestyle=":", alpha=0.7,
                    label=f"{model} median={med:.3f}")

    ax2.set_xlabel("Cosine distance (all pairs, all seeds)", fontsize=12)
    ax2.set_ylabel("Density", fontsize=12)
    ax2.set_title("Cosine distance distribution — CNN vs SAE (Global)",
                  fontsize=13, fontweight="bold")
...
def save_stats_csv(rows_by_model, summaries, out_dir):
    ...
    with open(raw_path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["model", "seed_idx", "class",
                                           "spearman", "pearson", "kendall", "n"])
        w.writeheader()
    ...
    for model, paths in sorted(seed_paths_by_model.items()):
        for seed_idx, path in enumerate(paths):
            ...
            try:
                d   = np.load(path, allow_pickle=True)
                cos = d["cosine_dist"].astype(np.float32)
                pt  = np.asarray(d.get("pair_type",
                                  np.full(len(cos), "intra")), dtype=str)
                ...
                q25, med, q75, p95, p99 = np.percentile(
                    samp, [25, 50, 75, 95, 99])
                row = dict(
                    model      = model,
                    seed_idx   = seed_idx,
...
def plot_global_scatter_gam(seed_paths, args, out_dir):
                ...
                try:
                    ...
                    for src_key, src_val in rc_data.items():
                        if isinstance(src_val, dict) and "error" not in src_val:
                            if "spearman_mean" in src_val: stats[model]["rw_spearman"].append(src_val["spearman_mean"])
                            if "kendall_mean" in src_val:  stats[model]["rw_kendall"].append(src_val["kendall_mean"])
                            if "pearson_mean" in src_val:  stats[model]["rw_pearson"].append(src_val["pearson_mean"])
                except Exception as e:
                    print(f"    Error reading {rc_jsons[0]}: {e}")
                    
            # 2. Read Global Pairs from combined.npz
            npz_cands = glob.glob(os.path.join(seed_dir, f"pairwise_{model}_combined.npz"))
            if not npz_cands:
            ...
            if npz_cands:
                try:
                    ...
                    if N_total > 0:
                        ...
                        idx_plot = rng.choice(N_total, min(N_total, n_sample_plot // len(paths)), replace=False)
    ...
    for col_i, model in enumerate(["CNN", "SAE"]):
        ...
        ax.scatter(x_cls, y_cls, c=color, alpha=args.alpha, s=args.point_size, linewidths=0, rasterized=True)
        
        has_gam = False
        adj_r2 = np.nan
        try:
            x_pred, y_line, y_lo, y_hi, adj_r2 = fit_gam_regression(
                x_cls, y_cls, n_splines=args.n_splines, lam=args.gam_lam, 
                degree=args.poly_degree, seed=args.seed,
        ...
        ax.set_title(f"{model}  │  All Classes (Inter + Intra)", fontsize=12, fontweight="bold")
        if has_gam:
            ax.legend(fontsize=9, loc="lower right")
        import seaborn as sns
        sns.despine(ax=ax)
...
def plot_seed_stability(seed_rows, args, out_dir):
    ...
    for ax, (stat, ylabel) in zip(axes, metrics):
        ...
        bp = ax.boxplot(bp_data, positions=range(len(models)),
        ...
        ax.set_title(f"Per-seed {ylabel}",
    ...
    fig.suptitle("Seed-to-seed variability of cosine distance distribution"
    ...
    n_models = len(models)
    fig2, axes2 = plt.subplots(1, n_models, figsize=(6.5 * n_models, 4.5),
                               sharey=False)
    if n_models == 1:
        axes2 = [axes2]
    ...
    for ax2, model in zip(axes2, models):
        ...
        for si, row in enumerate(r for r in seed_rows if r["model"] == model):
            ...
            if approx_sigma <= 0:
                continue
            x_range = np.linspace(
                max(0, approx_mu - 3 * approx_sigma),
                row["p99"] * 1.1, 300)
            from scipy.stats import norm
            y_kde = norm.pdf(x_range, approx_mu, approx_sigma)
            ax2.plot(x_range, y_kde, color=color_i, linewidth=1.6,
                     alpha=0.8, label=f"seed {si}")

        ax2.set_xlabel("Cosine distance (intra-class)", fontsize=11)
        ax2.set_ylabel("Density (approx)", fontsize=10)
        ax2.set_title(f"{model} — per-seed distribution",
                      fontsize=12, fontweight="bold")
        ax2.legend(fontsize=7, ncol=2)
        ax2.grid(True, alpha=0.12)
        sns.despine(ax=ax2)

    fig2.suptitle("Per-seed cosine distribution shape (approximate)",
                  fontsize=13, fontweight="bold")
    fig2.tight_layout()
...
def main():
    ...
    if args.check_version:
        ...
        print(f"  pygam backend: {'YES (LinearGAM)' if _HAS_PYGAM else 'NO  (scipy fallback)'}")
        sys.exit(0)
    ...
    print(f"  backend: {'pygam' if _HAS_PYGAM else 'scipy (install pygam for full GAM)'}")
    print(f"{'='*70}\n")
    ...
    for model, paths in seed_paths.items():
        ...
        cos, dpt, ci, cj = load_npz(path, args)
        print(f"    {len(cos):,} pairs  cosine range [{cos.min():.4f}, {cos.max():.4f}]")
    ...
    sw_data = {m: (tup[0], tup[1], tup[4])
               for m, tup in display_data.items()}
